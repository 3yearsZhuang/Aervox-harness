...
./aervox ci              # 本地门禁：ci-code（install+build+typecheck+test）+ ci-docs（lint）
...
| `mise tasks run ci-code` / `ci-docs` | 等同 `aervox ci` |

## 架构与技术栈

演进式模块化单体（ADR-001/014）+ Vue 全栈单栈（ADR-015）：桌面/Web/移动端共享契约与技术族。详细设计见 [架构设计](docs/ARCHITECTURE.md)（C4、数据所有权、可靠性、安全）与 [ADR](docs/architecture/adr/README.md)。
...
| 数据 | SQLite (WAL) + Drizzle + 仓储抽象，PG 双引擎兼容规划（[AVX-DB-001](docs/contracts/DATABASE.md)） |
| 后台 | Worker 进程（Outbox / 日记 / 删除），Redis + BullMQ 待接入 |

## 仓库结构

```text
apps/
  api/          Fastify /v1/ API，按领域模块组织（8 个 modules/*，见 ADR-014）
...
docs/           产品、需求、架构、契约、安全、运维与流程文档（详见下方「文档与追踪」）
```
...
- [PRD](docs/PRD.md)（AVX-PRD-001）· [架构设计](docs/ARCHITECTURE.md) · [ADR](docs/architecture/adr/README.md) · [SRS](docs/requirements/SRS.md)
- [需求追踪](docs/REQUIREMENTS_TRACEABILITY.md)：CAP 状态、DoR、G0~G6 门禁
- [可选功能模块化](docs/architecture/optional_modules.md) · [操作指南](docs/how-to/)

## 项目状态与质量门禁

- P0（CAP-001~013）需求已 `Specified` 并进入 DoR 评估；
- 首批落地：API 域路由 + SQLite 仓储（[AVX-DB-001](docs/contracts/DATABASE.md)）、Fairy 桌面端、Web 工作台、学习域契约、间隔重复调度。

合并到 `main` 前通过（本地 `./aervox ci` 等效，CI 定义见 `.github/workflows/`）：

- **代码**（`apps/**`、`packages/**`、锁文件等）：install + build + typecheck + test；
- **文档**（`docs/**`、`README.md` 等）：markdown lint + 链接检查。
