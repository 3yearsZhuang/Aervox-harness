...
export async function createSubagentRunsTables(client: Client): Promise<void> {
  // 4.8 阶段 5c：Subagent 运行关联（subagent_runs；AVX-HAR-001 §13 阶段 5c）
    ...
    `);
  await client.execute(`
      CREATE INDEX IF NOT EXISTS subagent_runs_local_parent_idx ON subagent_runs(parent_turn_id);
    `);
  await client.execute(`
      CREATE INDEX IF NOT EXISTS subagent_runs_local_session_idx ON subagent_runs(session_id);
    `);
  await client.execute(`
      CREATE UNIQUE INDEX IF NOT EXISTS subagent_runs_local_parent_exec_idx ON subagent_runs(parent_attempt_id, parent_execution_id);
    `);
}
