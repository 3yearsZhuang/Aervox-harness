...
export interface IAgentInboxRepository {
  ...
  enqueue(ctx: LocalContext, input: AgentInboxEnqueueInput): Promise<AgentInboxItemModel>;
  /**
  ...
  acknowledge(ctx: LocalContext, itemIds: string[]): Promise<void>;
  /** 按 idempotencyKey 查询（API 幂等返回用） */
  getByIdempotencyKey(ctx: LocalContext, idempotencyKey: string): Promise<AgentInboxItemModel | null>;
