...
export interface IVoiceRemoteConfigRepository {
  ...
  getConfig(ctx: LocalContext): Promise<RemoteVoiceConfigModel | null>;
  /** upsert 到激活行（无激活行则插入并激活）；返回保存后的模型 */
  saveConfig(ctx: LocalContext, input: RemoteVoiceConfigSaveInput): Promise<RemoteVoiceConfigModel>;
  /** 列出全部预设（含激活标记） */
  listPresets(ctx: LocalContext): Promise<RemoteVoiceConfigModel[]>;
  /** 新建预设（默认不激活，除非该租户尚无任何预设） */
  createPreset(ctx: LocalContext, name: string, input: RemoteVoiceConfigSaveInput): Promise<RemoteVoiceConfigModel>;
  /** 更新指定预设（保留激活状态） */
  updatePreset(ctx: LocalContext, presetId: string, input: RemoteVoiceConfigSaveInput): Promise<RemoteVoiceConfigModel | null>;
  /** 激活指定预设（事务内取消该租户其它激活）；预设不存在返回 null */
  activatePreset(ctx: LocalContext, presetId: string): Promise<RemoteVoiceConfigModel | null>;
