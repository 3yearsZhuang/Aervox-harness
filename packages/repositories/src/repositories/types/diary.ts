...
export interface IDiaryRepository {
  ...
  getCycle(tenant: LocalContext, cycleId: string): Promise<DiaryCycleModel | null>;
  claimCycleWithLease(
    tenant: LocalContext,
    params: {
      cycleId: string;
  ...
  getDiaryByDate(tenant: LocalContext, localDate: string): Promise<DiaryModel | null>;
  /** 历史回看：按日期倒序返回最近 limit 篇日记 */
  listDiaries(tenant: LocalContext, limit: number): Promise<DiaryModel[]>;
  ...
  getDiarySchedule(tenant: LocalContext, id: string): Promise<DiaryScheduleModel | null>;
  /** 默认每日计划幂等查询：返回该租户首个启用且 cutoffRule=daily 的计划（无返回 null） */
  getActiveDailySchedule(tenant: LocalContext): Promise<DiaryScheduleModel | null>;
  createDiaryVersion(
    tenant: LocalContext,
