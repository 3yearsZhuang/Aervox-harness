...
export interface ILLMConfigRepository {
  ...
  getConfig(ctx: LocalContext): Promise<LLMConfigModel | null>;
  ...
  /** 列出全部预设（含激活标记） */
  listPresets(ctx: LocalContext): Promise<LLMConfigModel[]>;
  /** 新建预设（默认不激活，除非该租户尚无任何预设） */
  createPreset(ctx: LocalContext, name: string, input: LLMConfigSaveInput): Promise<LLMConfigModel>;
  /** 更新指定预设（保留激活状态） */
  updatePreset(ctx: LocalContext, presetId: string, input: LLMConfigSaveInput): Promise<LLMConfigModel | null>;
  /** 激活指定预设（事务内取消该租户其它激活）；预设不存在返回 null */
  activatePreset(ctx: LocalContext, presetId: string): Promise<LLMConfigModel | null>;
