/**
 * Aervox｜思隅 @aervox/repositories — 日记与调度周期 SQLite 仓储实现
 *
...
export class SqliteDiaryRepository implements IDiaryRepository {
  ...
  async getCycle(ctx: LocalContext, cycleId: string): Promise<DiaryCycleModel | null> {
  ...
  async getDiaryByDate(ctx: LocalContext, localDate: string): Promise<DiaryModel | null> {
    const [found] = await this.db
      .select()
      .from(diaries)
      .where(
  ...
  async listDiaries(ctx: LocalContext, limit: number): Promise<DiaryModel[]> {
  ...
  async getDiarySchedule(ctx: LocalContext, id: string): Promise<DiaryScheduleModel | null> {
    const [found] = await this.db
      .select()
      .from(diarySchedules)
      .where(
  ...
  async getActiveDailySchedule(ctx: LocalContext): Promise<DiaryScheduleModel | null> {
    const [found] = await this.db
      .select()
      .from(diarySchedules)
      .where(
  ...
  async createDiaryVersion(
    ...
    if (!diary) throw new Error(`Diary ${versionData.diaryId} not found in ctx`);
    const [created] = await this.db
  ...
  async updateDiaryContent(
    ...
    if (!existing) throw new Error(`Diary ${diaryId} not found in ctx`);
    const now = new Date().toISOString();
