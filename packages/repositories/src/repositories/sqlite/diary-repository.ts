...
export class SqliteDiaryRepository implements IDiaryRepository {
  ...
  async getCycle(tenant: LocalContext, cycleId: string): Promise<DiaryCycleModel | null> {
    assertLocalContext(tenant);
  ...
  async getDiaryByDate(tenant: LocalContext, localDate: string): Promise<DiaryModel | null> {
    assertLocalContext(tenant);
    const [found] = await this.db
      .select()
      .from(diaries)
  ...
  async listDiaries(tenant: LocalContext, limit: number): Promise<DiaryModel[]> {
    assertLocalContext(tenant);
  ...
  async getDiarySchedule(tenant: LocalContext, id: string): Promise<DiaryScheduleModel | null> {
    assertLocalContext(tenant);
    const [found] = await this.db
      .select()
      .from(diarySchedules)
  ...
  async getActiveDailySchedule(tenant: LocalContext): Promise<DiaryScheduleModel | null> {
    assertLocalContext(tenant);
    const [found] = await this.db
      .select()
      .from(diarySchedules)
