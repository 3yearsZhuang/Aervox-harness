...
const ctx: LocalContext = { workspaceId: "ws_lease", subjectUserId: "usr_lease" };
...
describe("3b-A 租约 TTL 与续租", () => {
  ...
  beforeEach(async () => {
    ...
    await repo.getOrCreateSession(ctx, "ses_lease", "租约测试");
    ...
    await repo.createTurnAttempt(ctx, "turn_lease", { id: "atp_lease", attempt: 1 });
  });
  ...
  it("续租：CAS 匹配（正确 leaseId + fencing）刷新过期时刻", async () => {
    ...
    const ok = await repo.renewTurnAttemptLease(ctx, {
      attemptId: "atp_lease",
      leaseId: "lease_1",
    ...
    expect(ok).toBe(true);
    const [attempt] = await repo.listTurnAttempts(ctx, "turn_lease");
  ...
  it("续租失败：leaseId 不符且 fencing 递增后旧期望值不再有效", async () => {
    ...
    // 错误 leaseId
    expect(
      await repo.renewTurnAttemptLease(ctx, { attemptId: "atp_lease", leaseId: "lease_wrong", expectedFencingToken: 1 }),
    ).toBe(false);
    // 旧 fencing 期望值（claim 后已 1，用 0 续租应失败）
    expect(
      await repo.renewTurnAttemptLease(ctx, { attemptId: "atp_lease", leaseId: "lease_1", expectedFencingToken: 0 }),
    ).toBe(false);
  });
  ...
  it("3b-B 抢占：租约未过期不可被抢占", async () => {
    ...
    const takeover = await repo.claimTurnAttempt(ctx, {
      turnId: "turn_lease",
      attemptId: "atp_lease",
      expectedFencingToken: 1,
      leaseId: "lease_2",
  ...
  it("3b-B 单一终态：finalize 后再次提交被拒绝", async () => {
    ...
    const first = await repo.finalizeTurnAttempt(ctx, {
      turnId: "turn_lease",
      attemptId: "atp_lease",
      status: "Completed",
      expectedFencingToken: 1,
    ...
    const second = await repo.finalizeTurnAttempt(ctx, {
      turnId: "turn_lease",
      attemptId: "atp_lease",
      status: "Failed",
      expectedFencingToken: 1,
...
describe("2b 取消请求位（CancelRequested）", () => {
  ...
  beforeEach(async () => {
    ...
    await initDatabaseSchema(client);
    repo = new SqliteConversationRepository(db);
    await repo.getOrCreateSession(ctx, "ses_cancel", "取消测试");
    await repo.createTurnWithOutbox(
      ctx,
    ...
    await repo.createTurnAttempt(ctx, "turn_cancel", { id: "atp_cancel", attempt: 1 });
  });

  it("Running attempt 可置取消请求位，turns 同步 Cancelled（未终态）", async () => {
    const res = await repo.requestCancelTurnAttempt(ctx, { turnId: "turn_cancel", attemptId: "atp_cancel" });
    expect(res.ok).toBe(true);
    expect(await repo.getTurnAttemptStatus(ctx, { turnId: "turn_cancel", attemptId: "atp_cancel" })).toBe("CancelRequested");
    const [attempt] = await repo.listTurnAttempts(ctx, "turn_cancel");
  ...
  it("已终态拒绝取消：finalize Completed 后返回 already_finalized，不覆盖", async () => {
    ...
    await repo.finalizeTurnAttempt(ctx, {
      turnId: "turn_cancel",
      attemptId: "atp_cancel",
      status: "Completed",
      expectedFencingToken: 1,
    ...
    expect(await repo.getTurnAttemptStatus(ctx, { turnId: "turn_cancel", attemptId: "atp_cancel" })).toBe("Completed");
  });

  it("CancelRequested 可提交终态（Cancelled），且单一终态仍成立", async () => {
    await repo.requestCancelTurnAttempt(ctx, { turnId: "turn_cancel", attemptId: "atp_cancel" });
    ...
    expect(second).toBeNull();
    expect(await repo.getTurnAttemptStatus(ctx, { turnId: "turn_cancel", attemptId: "atp_cancel" })).toBe("Cancelled");
  });

  it("未知 attempt 返回 not_found（查询状态为 null）", async () => {
    const res = await repo.requestCancelTurnAttempt(ctx, { turnId: "turn_cancel", attemptId: "atp_missing" });
    expect(res).toEqual({ ok: false, reason: "not_found" });
    expect(await repo.getTurnAttemptStatus(ctx, { turnId: "turn_cancel", attemptId: "atp_missing" })).toBeNull();
  });
});