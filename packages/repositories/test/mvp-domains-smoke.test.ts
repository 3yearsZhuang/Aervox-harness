...
describe("PRD §8 MVP 优先队列：新域仓储冒烟测试", () => {
  ...
  it("学习域：目标/题目/作答/知识点/复习项可写可读，且本地上下文共享", async () => {
    const goal = await learning.createLearningGoal(ctx, { id: "goal_1", topic: "代数", level: "intermediate", availableMinutes: 30 });
    expect(goal.status).toBe("active");
    expect((await learning.listLearningGoals(ctx))).toHaveLength(1);

    const q = await learning.createQuestion(ctx, { id: "q_1", prompt: "2+2=?", answerSpec: { answer: "4" } });
    expect(q.answerSpec).toEqual({ answer: "4" });

    const attempt = await learning.recordAttempt(ctx, {
      id: "att_1", sessionId: "ses_1", questionId: "q_1", answer: "4", judgement: "correct",
    });
    expect(attempt.judgement).toBe("correct");
    expect(await learning.listAttemptsByQuestion(ctx, "q_1")).toHaveLength(1);

    const ki = await learning.createKnowledgeItem(ctx, { id: "ki_1", concept: "加法", sourceStatus: "observed" });
    const mastered = await learning.updateMastery(ctx, "ki_1", "mastered", { basis: "attempts" });
    expect(mastered?.masteryState).toBe("mastered");

    const ri = await learning.createReviewItem(ctx, { id: "ri_1", knowledgeId: "ki_1", dueAt: "2026-01-01T00:00:00.000Z" });
    expect(await learning.listDueReviewItems(ctx, "2026-12-31T00:00:00.000Z")).toHaveLength(1);
    expect((await learning.completeReviewItem(ctx, "ri_1"))?.status).toBe("completed");
  ...
  it("反馈域：写入并可按主体过滤", async () => {
    await feedback.createFeedback(ctx, { id: "fb_1", actorId: "usr_mvp", subjectType: "message", subjectId: "m_1", type: "inaccurate" });
    const all = await feedback.listFeedback(ctx);
    expect(all).toHaveLength(1);
    const filtered = await feedback.listFeedback(ctx, "message", "m_1");
    expect(filtered).toHaveLength(1);
    expect(await feedback.listFeedback(otherTenant)).toHaveLength(1);
  });

  it("溯源/记忆：来源工件 + 修订 + 记忆版本/证据/事件可写，来源删除保留 tombstone", async () => {
    const sa = await provenance.createSourceArtifact(ctx, {
      id: "sa_1", kind: "message", ownerModule: "conversation", occurredAt: "2026-01-01T00:00:00.000Z", ingestedAt: "2026-01-01T00:00:01.000Z",
    });
    const rev = await provenance.appendSourceRevision(ctx, "sa_1", { id: "sr_1", checksum: "abc123", content: "原文内容" });
    expect(rev.checksum).toBe("abc123");
    await provenance.setCurrentRevision(ctx, "sa_1", "sr_1");
    expect((await provenance.getSourceArtifact(ctx, "sa_1"))?.currentRevisionId).toBe("sr_1");

    const mem = await memory.createRecord(ctx, { id: "mem_1", layer: "long_term", type: "user_fact", content: "用户目标是考雅思 7 分" });
    const mrev = await provenance.appendMemoryRevision(ctx, { id: "mrev_1", memoryId: "mem_1", content: "用户目标：雅思 7 分", confidence: 90 });
    await provenance.setMemoryCurrentRevision(ctx, "mem_1", "mrev_1");
    expect(await provenance.listMemoryRevisions(ctx, "mem_1")).toHaveLength(1);

    await provenance.createMemoryEvidence(ctx, { id: "mev_1", memoryRevisionId: "mrev_1", sourceArtifactId: "sa_1", sourceRevisionId: "sr_1" });
    await provenance.recordMemoryEvent(ctx, { id: "mevt_1", memoryId: "mem_1", action: "promoted", fromTier: "short_term", toTier: "long_term" });
    const events = await provenance.listMemoryEvents(ctx, "mem_1");
    expect(events).toHaveLength(1);
    expect(events[0]!.action).toBe("promoted");
  });

  it("平台域：计划任务/通知/提示词/模型运行/上下文清单/审计", async () => {
    await platform.createScheduledJob(ctx, { id: "job_1", jobType: "diary", subjectId: "sub_1", idempotencyKey: "j1", runAt: "2026-01-01T00:00:00.000Z" });
    expect((await platform.markJobDone(ctx, "job_1"))?.status).toBe("done");

    await platform.createNotification(ctx, { id: "ntf_1", type: "review", scheduledAt: "2026-01-01T00:00:00.000Z", channel: "in_app" });
    expect((await platform.markNotificationSent(ctx, "ntf_1"))?.status).toBe("sent");
    ...
    const run = await platform.createModelRun(ctx, { id: "run_1", purpose: "diary", provider: "openai", modelId: "gpt-4o", promptVersionId: "pv_1" });
    ...
    await platform.attachContextManifest(ctx, "run_1", "cm_1");
    expect((await platform.completeModelRun(ctx, "run_1", { latencyMs: 120, cost: 10, tokenUsage: { total: 100 } }))?.status).toBe("completed");

    await platform.createAuditRecord(ctx, { id: "aud_1", actorType: "user", actorId: "usr_mvp", action: "export", subjectType: "diary", subjectId: "d_1" });
    expect(await platform.listAuditRecords(ctx)).toHaveLength(1);
  });

  it("安全域 + 隐私/删除域：事件记录、同意授权、删除请求传播", async () => {
    await safety.recordIncident(ctx, { id: "saf_1", category: "privacy", severity: "high", disposition: "blocked", policyVersion: "v1" });
    expect(await safety.listIncidents(ctx)).toHaveLength(1);

    await privacy.grantConsent(ctx, { id: "cg_1", actorId: "usr_mvp", purpose: "diary", scope: "auto_generate", policyVersion: "v1" });
    expect(await privacy.hasActiveConsent(ctx, "diary", "auto_generate")).toBe(true);
    await privacy.revokeConsent(ctx, "cg_1");
    expect(await privacy.hasActiveConsent(ctx, "diary", "auto_generate")).toBe(false);

    const req = await privacy.createDeletionRequest(ctx, { id: "dr_1", scope: "account", idempotencyKey: "del_1", ownerModule: "account" });
  ...
  it("会话补齐：Message 身份 + TurnAttempt", async () => {
    const session = await conversation.createSession(ctx, "MVP Session");
    const msg = await conversation.createMessage(ctx, { id: "msg_1", sessionId: session.id, role: "user", label: "首问" });
    expect(msg.role).toBe("user");
    expect((await conversation.getMessage(ctx, "msg_1"))?.label).toBe("首问");
    ...
    const attempt = await conversation.createTurnAttempt(ctx, turn.turn.id, { id: "ta_1", attempt: 1, leaseId: "lease_1" });
    expect(attempt.attempt).toBe(1);
    expect(await conversation.listTurnAttempts(ctx, "turn_1")).toHaveLength(1);
  });
  ...
  it("日记域补齐：计划主实体/版本/段落来源/素材缓冲", async () => {
    const schedule = await diary.createDiarySchedule(ctx, {
      id: "ds_1",
    ...
    expect((await diary.getDiarySchedule(ctx, "ds_1"))?.bufferMinutes).toBe(15);

    // 先创建不可变周期（scheduleVersion=1），再在同一事务发布日记
    const cycle1 = await diary.createCycle(ctx, {
      id: "cyc_1",
      scheduleEpochId: "epoch_1",
    ...
    const created = await diary.publishDiaryWithCycle(ctx, {
      cycleId: "cyc_1",
    ...
    const dv = await diary.createDiaryVersion(ctx, { id: "dv_1", diaryId: "d_1", perspective: "ai_generated", content: "v1" });
    ...
    const cycle = await diary.createCycle(ctx, {
      id: "cyc_2",
      scheduleEpochId: "epoch_1",
    ...
    const buf = await diary.createDiaryMaterialBuffer(ctx, {
      id: "dmb_1",
      cycleId: cycle.id,
  ...
  it("埋点域 + 内容域：AnalyticsEvent / Attachment / EmbeddingIndex", async () => {
    await analytics.recordEvent(ctx, {
      id: "ae_1",
      eventName: "review_completed",
      analyticsSubjectId: "pseudo_1",
      context: { count: 1 },
    });
    const events = await analytics.listEventsBySubject(ctx, "pseudo_1");
    expect(events).toHaveLength(1);
    expect(events[0]!.privacyClass).toBe("normal");

    const att = await content.createAttachment(ctx, { id: "att_1", objectKey: "obj/1", mediaType: "image/png", size: 1024 });
    expect(att.scanStatus).toBe("pending");
    expect((await content.getAttachment(ctx, "att_1"))?.objectKey).toBe("obj/1");

    const idx = await content.createEmbeddingIndex(ctx, {
      id: "ei_1",
      sourceArtifactId: "sa_1",
      sourceRevisionId: "sr_1",
    ...
    expect(await content.listEmbeddingIndexes(ctx, "sa_1")).toHaveLength(1);
  });
});
