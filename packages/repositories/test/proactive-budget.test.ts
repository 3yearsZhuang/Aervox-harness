...
const ctx = { workspaceId: "ws_budget", subjectUserId: "usr_budget" } as const;
...
describe("CR-033 E2b 预算 repo（CAS / 幂等）", () => {
  ...
  it("初始化幂等：重复 getOrInit 不重复创建", async () => {
    const first = await repo.getOrInitBudget(ctx, "plugin", "health-guard");
    const second = await repo.getOrInitBudget(ctx, "plugin", "health-guard");
    expect(second.id).toBe(first.id);
    expect(second.budgetUnits).toBe(first.budgetUnits);
  });

  it("CAS 扣减成功递减余额并递增版本", async () => {
    const row = await repo.getOrInitBudget(ctx, "global", null);
    const outcome = await repo.reserveBudget(ctx, "global", null, row.reserveVersion, 20);
  ...
  it("版本冲突拒绝（并发防超发）", async () => {
    const row = await repo.getOrInitBudget(ctx, "global", null);
    await repo.reserveBudget(ctx, "global", null, row.reserveVersion, 20);
    // 用旧版本再扣 → CAS 冲突
    const stale = await repo.reserveBudget(ctx, "global", null, row.reserveVersion, 20);
    expect(stale.ok).toBe(false);
    expect(stale.budget?.budgetUnits).toBe(80);
  });

  it("余额不足拒绝（不透支）", async () => {
    const row = await repo.getOrInitBudget(ctx, "plugin", "p1");
    const outcome = await repo.reserveBudget(ctx, "plugin", "p1", row.reserveVersion, row.budgetUnits + 1);
  ...
  it("退款回补且封顶上限", async () => {
    const row = await repo.getOrInitBudget(ctx, "global", null);
    const reserved = await repo.reserveBudget(ctx, "global", null, row.reserveVersion, 20);
    const refunded = await repo.refundBudget(ctx, "global", null, reserved.budget!.reserveVersion, 500);
    expect(refunded.ok).toBe(true);
    expect(refunded.budget?.budgetUnits).toBe(100); // maxUnits 封顶
  });

  it("全局与插件预算同一事务原子预留并原子退款", async () => {
    const global = await repo.getOrInitBudget(ctx, "global", null);
    const plugin = await repo.getOrInitBudget(ctx, "plugin", "p-pair");
    const reserved = await repo.reserveBudgetPair(ctx, "p-pair", {
    ...
    const refunded = await repo.refundBudgetPair(ctx, "p-pair", {
  ...
  it("双预算任一 CAS 冲突时全部回滚", async () => {
    const global = await repo.getOrInitBudget(ctx, "global", null);
    const plugin = await repo.getOrInitBudget(ctx, "plugin", "p-rollback");
    const failed = await repo.reserveBudgetPair(ctx, "p-rollback", {
  ...
  it("反馈应用：忽略收缩持久化", async () => {
    await repo.getOrInitBudget(ctx, "plugin", "p2", { ignoreThreshold: 1 });
    const result = await repo.applyFeedback(ctx, "plugin", "p2", feedback("ignored"), { ignoreThreshold: 1 });
  ...
  it("反馈幂等：重复事件只影响预算一次", async () => {
    await repo.getOrInitBudget(ctx, "plugin", "p3", { ignoreThreshold: 1 });
    const event = feedback("ignored", 1, "fb_idempotent");
    const first = await repo.applyFeedback(ctx, "plugin", "p3", event, { ignoreThreshold: 1 });
    const second = await repo.applyFeedback(ctx, "plugin", "p3", event, { ignoreThreshold: 1 });
  ...
  it("回执幂等：同幂等键重复写入只保留一条", async () => {
    ...
    const first = await repo.saveReceipt(ctx, base);
    const second = await repo.saveReceipt(ctx, { ...base, budgetAfter: 999 });
    expect(first.created).toBe(true);
    expect(second.created).toBe(false);
    expect(second.receipt.budgetAfter).toBe(80);
    const listed = await repo.listReceiptsByAction(ctx, "action_1");
