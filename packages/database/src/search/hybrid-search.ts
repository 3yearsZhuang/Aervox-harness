...
export interface HybridSearchStorage {
  ftsSearch(queryText: string, topK: number): Promise<HybridChannelHit[]>;
  vectorSearch(
    queryVector: number[],
    topK: number,
    minScore?: number,
...
export class HybridSearchService {
  ...
  async search(input: HybridSearchInput): Promise<HybridSearchHit[]> {
    ...
    const ftsPromise = input.queryText.trim()
      ? this.storage.ftsSearch(input.queryText, topK)
      : Promise.resolve([]);
...
export function createHybridSearchStorage(opts: {
  ...
  async function ftsSearch(
    ...
    if (domain === "memory") {
      ...
      return searchMemoriesFts(client, queryText, topK);
    }
    const { searchMessagesFts } = await import("./fts.js");
    return searchMessagesFts(client, queryText, topK);
  }
  ...
  async function vectorSearch(
    ...
    const res = await vectorPort.search(queryVector, topK, minScore);
