...
export class SqliteProactiveIntelligenceRepository {
  ...
  async listRelationships(tenant: TenantContext, limit?: number) {
    ...
    const rows = await this.db.select().from(proactiveRelationships).orderBy(desc(proactiveRelationships.lastInteractionAt)).limit(limitOf(limit));
  ...
  async createCommitment(tenant: TenantContext, input: Omit<IntelligenceCommitment, "createdAt" | "updatedAt">): Promise<IntelligenceCommitment> {
    ...
    const [row] = await this.db.insert(proactiveCommitments).values({
      id: input.id, revisionId: input.revisionId, projectId: input.projectId ?? null, relationshipId: input.relationshipId ?? null,
  ...
  async recordTriggerEvent(tenant: TenantContext, input: {
    ...
    const [row] = await this.db.insert(proactiveTriggerEvents).values({
      id: input.id, revisionId: input.revisionId, ruleId: input.ruleId ?? null, triggerType: input.triggerType,
  ...
  async listTriggerEvents(tenant: TenantContext, limit?: number) {
    ...
    const rows = await this.db.select().from(proactiveTriggerEvents).orderBy(desc(proactiveTriggerEvents.occurredAt)).limit(limitOf(limit));
  ...
  async createClaimConflict(tenant: TenantContext, input: {
    ...
    const [row] = await this.db.insert(proactiveClaimConflicts).values({
      id: input.id, revisionId: input.revisionId, primaryClaimId: input.primaryClaimId, conflictingClaimId: input.conflictingClaimId,
  ...
  async listAttentionStates(tenant: TenantContext, limit?: number) {
    ...
    const rows = await this.db.select().from(proactiveAttentionStates).orderBy(desc(proactiveAttentionStates.windowEnd)).limit(limitOf(limit));
  ...
  async listScenes(tenant: TenantContext, limit?: number) {
    ...
    const rows = await this.db.select().from(proactiveSceneSnapshots).orderBy(desc(proactiveSceneSnapshots.capturedAt)).limit(limitOf(limit));
  ...
  async listReviews(tenant: TenantContext, limit?: number) {
    ...
    const rows = await this.db.select().from(proactiveReviewReports).orderBy(desc(proactiveReviewReports.periodEnd)).limit(limitOf(limit));
