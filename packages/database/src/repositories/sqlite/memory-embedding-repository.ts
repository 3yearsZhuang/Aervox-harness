...
export class SqliteMemoryVectorSearchAdapter {
  ...
  async upsert(
    ...
    await this.repo.insertBatch({} as TenantContext, [
      ...items.map((item) => ({
        id: `vec_${item.id}`,
        memoryId: item.id,
        vector: item.vector,
  ...
  async search(
    ...
    const hits = await this.repo.retrieve({} as TenantContext, queryVector, topK, minScore, this.modelId);
    return hits.map((h) => ({ id: h.memoryId, score: h.score }));
  }

  async delete(id: string): Promise<void> {
    await this.repo.deleteByMemoryId({} as TenantContext, id);
  }

  async clear(): Promise<void> {
    await this.repo.clearTenant({} as TenantContext);
  }
}