...
describe("TC-PRIV-DEL-001: 删除传播与即刻零召回验证测试", () => {
  ...
  it("删除消息后，业务记录、FTS5 全文索引与向量检索立即实现零召回", async () => {
    ...
    await indexMessageFts(client, { id: messageId, content: sensitiveText });
    await vectorPort.upsert([
      { id: messageId, vector: [0.1, 0.9, 0.4], metadata: { content: sensitiveText } },
    ]);

    // 3. 验证删除前能够检索召回
    const ftsBefore = await searchMessagesFts(client, "sunshine123");
    expect(ftsBefore).toHaveLength(1);
    expect(ftsBefore[0]!.id).toBe(messageId);

    const vectorBefore = await vectorPort.search([0.1, 0.9, 0.4], 5);
    ...
    await deleteMessageFts(client, messageId);
    await vectorPort.delete(messageId);

    // 5. 验证删除后：FTS5 与 Vector 零召回
    const ftsAfter = await searchMessagesFts(client, "sunshine123");
    expect(ftsAfter).toHaveLength(0);

    const vectorAfter = await vectorPort.search([0.1, 0.9, 0.4], 5);
