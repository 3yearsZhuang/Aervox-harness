...
describe("CAP-033 proactive profile repository", () => {
  ...
  it("keeps an unacknowledged OS grant requested", async () => {
  ...
  it("marks optional platform sources as non-mandatory in source manifest", async () => {
    const repository = new SqliteProactiveProfileRepository(db);
  ...
  it("derives active once mandatory sources are granted even when platform-pending sources wait", async () => {
    ...
    await repository.createActivationLease(tenant, {
      id: "lease_pending",
      revisionId: result.revision.id,
      deviceId: "device-b",
      epoch: "epoch-pending",
