...
describe('PluginManagerPanel.vue Actions Layout & State', () => {
  it('places config button to the left of switch, fixes switch on the far right, and disables config button when plugin is disabled', async () => {
    ...
    expect(focusActionButtons).toHaveLength(2);

    // Left button: config button
    ...
    const focusToggle = focusActionButtons[1];
    ...
    expect(healthActionButtons).toHaveLength(2);

    // Left button: config button is STILL present, but disabled
