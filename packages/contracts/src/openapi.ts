...
const scopeHeaders = z.object({
  ...
  "X-Subject-User-Id": z.string().min(1).optional(),
});
...
registry.registerPath({ method: "get", path: "/v1/mcp/tools", summary: "列出 MCP 工具", tags: ["MCP"], responses: { 200: { description: "MCP tools", content: { "application/json": { schema: z.object({ tools: z.array(mcpToolSchema) }) } } } } });
...
export const openApiDocument: OpenAPIObject = generator.generateDocument({
  ...
  info: {
    title: "Aervox｜思隅 API",
    version: "0.2.0",
    description: "Turn streaming plus Persona, Anthropic Skills, MCP policy, and GPT-SoVITS contracts.",
  },
