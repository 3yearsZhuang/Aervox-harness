...
3. [文档治理规范](docs/reference/document-governance.md) — 事实源、状态模型、owner、复核触发与迁移阶段；
4. [教程：构建并运行第一个对话](docs/tutorials/first-conversation.md)；
5. [文档写作规范](docs/reference/standards/doc-standards.md)（目录=Diátaxis 四分类、文档头 schema、命名、Vale 门禁）与[术语表](docs/reference/standards/terminology.md)；
6. 按需深入：PRD / 架构与 ADR / 契约（流式协议·数据库）。
...
- SQLite/libsql 事务与读一致：写操作走写者连接；事务提交后其它连接存在读快照滞后，测试断言须用写者连接自身；事务内不嵌套、不长时间持锁；libsql 0.4.x 事务 BEGIN 阶段不会自动重试（详见 [ADR-003](docs/reference/adr/ADR-003-postgres-retrieval.md) 与 `packages/repositories`）；
...
mise tasks run ci-docs    # 改动 docs/** README.md CONTRIBUTING.md AGENTS.md 后必跑
