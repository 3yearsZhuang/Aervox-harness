...
export function registerPluginConfigRoutes(app: FastifyInstance, service: PluginConfigService): void {
  ...
  app.get("/v1/plugins/:pluginId/pages/:pageId/assets/*", async (req, reply) => {
    ...
    try {
      ...
      return reply
        .header("Content-Security-Policy", "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'none'; frame-ancestors 'self'; base-uri 'none'; form-action 'none'")
