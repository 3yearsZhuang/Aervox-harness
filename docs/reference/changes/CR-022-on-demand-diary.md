...
- 验证：`apps/api/test/diary-ondemand.test.ts` 集成测试 5 项（PET-05 未授权拒绝 / 授权新建 + `GET /v1/diaries` / 同日改写版本与主行推进 / 无素材空日记诚实降级 / `createRuntimeToolProvider` 审批缝挂起-授权-执行）；`openapi-contract.test.ts` 契约断言；全仓 `pnpm build`/`typecheck`/`test` 通过；本地运行栈实测（工具注册 `write_with_approval`+`diary` 分类、OpenAPI 路径、空态 404）。
- 决策：Review Candidate，阶段 1（对话触发）已落地；定时路径按阶段 2 推进。
