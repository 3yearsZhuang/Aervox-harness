...
- 已验证：`@aervox/schema`、`@aervox/repositories`、`@aervox/api`、`@aervox/worker` 构建与定向测试；仓储 41 个测试文件共 203 项、API 48 个测试文件共 330 项、Worker 4 个测试文件共 10 项、Host Agent 12 个测试文件共 66 项通过（2 项真实模型/库内构建条件测试跳过）；全仓 `mise tasks run ci-code` 与 `ci-docs` 通过。
...
- 已落地：API 请求边界固定生成本地单用户上下文，不再解析 `x-workspace-id`、`x-user-id` 或 `x-actor-id`；token 模式不再要求租户绑定配置，Web、Desktop 与共享 API Client 也不再发送租户 Header。
- 兼容边界：`LocalContext` 与 `workspaceId`/`subjectUserId` 仍存在于部分 Port 和调用方签名，但只用于保持调用兼容或迁移范围输入，不构成数据库安全边界，也不写入最终库。
- 发布前门禁：DSH 固定 SHA、许可证与协议回合探测已通过；跨进程停写协调、生产 FTS 重建与 rollback 保留仍需在完整运行编排中复核。上述运行演练不影响本次去租户化实现，但完成前不得将 CR 标记为 `released`。
