...
| 专注模式与术语下钻模块化（AVX-PLUG-001 第一方插件模块） | CAP-007/020 / ADR-009 / ADR-015 | `packages/ui/src/modules/study-mode/`（`StudyModeSwitch.vue`、`StudyTermsBar.vue`、`TermExploreDialog.vue`、`index.ts`）、`packages/ui/src/components/{AervoxWorkbench.vue,workbench/WorkbenchHeader.vue,workbench/ConversationConsole.vue,workbench/TermsBar.vue,TermExploreDialog.vue}`、`packages/ui/src/registry/index.ts`、`packages/ui/test/study-mode-module.test.ts` | 2026-09-09 | `@aervox/ui` 单元测试 22/22 全绿（含 `study-mode-module.test.ts` 插槽注册/优先级/注销/幂等性测试）；`@aervox/ui`、`@aervox/web`、`@aervox/desktop` vue-tsc 检查零错误；向后兼容导出校验通过 | 原生 + `AVX-PLUG-001` 规范 |
