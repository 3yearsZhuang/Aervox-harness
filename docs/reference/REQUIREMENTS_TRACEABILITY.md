...
| `CAP-027` | 本地数据主权与可移植性 | `P2 · R4` | `Specified` | Not Ready | ✔ | [P2 验收原则](PRD.md#prd-cap-020-027)、[CR-030](changes/CR-030-pure-local-sqlite-database.md) | D0 已批准本地单用户 SQLite、导出、备份、恢复和显式破坏性迁移；D1～D3 迁移器、回滚和安全门禁完成前保持 Not Ready |
...
| CR-030 D1 备份清单切片：业务库/Vault/RecoveryControlLedger/WAL artifact 复制、SHA-256 manifest 与篡改检测 | CR-030 / NFR-DR-001 / NFR-REL-001 | `packages/repositories/src/migration/backup-manifest.ts`、`packages/repositories/src/migration/index.ts`、`packages/repositories/test/backup-manifest.test.ts` | 2026-09-11 | CR-030 定向 Vitest 2 tests；`@aervox/repositories` typecheck；仅交付 artifact 清单与校验，停写协调、SQLite 一致性快照、staging 和原子换库仍待完成 | 原生 |
| CR-030 D1 staging/换库切片：外键拓扑复制、选定范围过滤、完整性/外键/行数校验与同文件系统原子替换 | CR-030 / NFR-DR-001 / NFR-REL-001 | `packages/repositories/src/migration/staging-migration.ts`、`packages/repositories/src/migration/index.ts`、`packages/repositories/test/staging-migration.test.ts` | 2026-09-11 | CR-030 定向 Vitest 2 tests；`@aervox/repositories` typecheck；跨进程停写协调、FTS 重建、生产 rollback 保留策略仍需接入运行编排 | 原生 |
...
| CR-030 D2 过渡切片：Repository/API/Worker 契约从 `TenantContext` 更名为 `LocalContext` | CR-030 / SEC-LOC-001 | `packages/repositories/src/local-context.ts`、`packages/repositories/src/index.ts`、`packages/repositories/src/repositories/{types,sqlite}/`、`apps/api/src/`、`apps/worker/src/`、`packages/host-agent/src/` | 2026-09-11 | API/Repository build、类型检查、租户兼容回归测试；旧 `workspace_id`/`subject_user_id` 列和过滤仍为迁移期兼容，不能视为 D2 完成 | 原生 |
