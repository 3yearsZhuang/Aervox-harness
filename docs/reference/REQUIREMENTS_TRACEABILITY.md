...
|---|---|---|---|---|---|
| 确立 SQLite 为永久纯本地真源，废弃 PostgreSQL 支持并移除多租户结构（CR-030）：全库业务表移除 `(workspace_id, subject_user_id)` 租户列与租户索引；仓储层移除 `TenantContext` 参数约束与应用层 RLS；持久层收敛为单一精简包 `@aervox/database`，API 与工作线程完全转向本地单用户运行时 | CAP-027 | `packages/database/src/{schema,repositories,client,index}.ts`、`apps/api/src/`、`apps/worker/src/`、`packages/host-agent/src/`、`packages/diary/src/`、`docs/reference/changes/CR-030-pure-local-sqlite-database.md` | 2026-09-10 | 全仓 `mise tasks run ci-code`、`mise tasks run ci-docs`、`import-boundary.mjs` | 原生（CR-030） |
...
| 开屏引导取材宣讲包改造与内嵌完整产品介绍入口（引导第 2/3 步换为六能力辐射图与学习闭环；设置 → 外观新增「完整产品介绍」内嵌播放） | CAP-001/018 | `apps/desktop/src/renderer/src/{App.vue,components/OnboardingFlow.vue}`、`packages/ui/src/components/AervoxWorkbench.vue`（`open-intro-deck` 事件）、`apps/desktop/src/renderer/public/aervox-intro/`（自研宣讲包 `Aervox-Siyu-20260829` 的内嵌裁剪版，来源/版权/裁剪说明见其 `README.md`） | 2026-08-30 | Desktop/UI typecheck 与 Vitest；Desktop build（`electron-vite`）；内嵌版浏览器烟测（10 页、翻页、入场动效、WebGL 背景） | 原生（取材自自研宣讲包；内嵌第三方运行库 `motion`/`lucide` 来源与许可见 `aervox-intro/README.md`） |
...
| 日记重构：AI 依据当天记忆与上下文模仿人类书写（共享包 @aervox/diary）＋ Worker 定时真实生成（非占位）＋ 首开临时卡片 ＋ 菜单栏日记本（回看/手动生成）（CAP-009 扩展；取代 #118 移除的「今日日记」卡 UI 形态） | CAP-009 | `packages/diary/`（新共享包：`material.ts` 素材含当日 `memory_records`、`prompts.ts` 人类随笔风格、`template.ts`、`generation.ts` 生成服务 + LLM 配置端口 + 本地日期）、`packages/database`（`diary-repository.ts` 新增 `listDiaries`/`getActiveDailySchedule`、`createDiarySchedule` 增 `enabled`/`nextRunAt`；`repositories/types.ts` 接口对齐）、`packages/contracts/src/{schemas,index,openapi}.ts`（`diaryListResponseSchema`/`diaryGenerateTodayOutputSchema`）、`apps/api/src/modules/diary/`（`application.ts` 应用服务 ensure/rewrite/默认计划懒建、`routes.ts` GET 列表 + `POST /v1/diaries/generate-today`、`index.ts` 装配 @aervox/diary、`diary-write-tool.ts` 收敛为应用服务薄封装；删 `generation.ts`/`material.ts`）、`apps/worker`（`diary-generator.ts` 定时真实生成 + 同日已有跳过推进游标、`index.ts` 注入 `SqliteLLMConfigRepository`）、`packages/api-client/src/useAervoxDiary.ts`（generateToday/getDiaryByDate/listDiaries + 导出）、`packages/ui/src/components/AervoxWorkbench.vue`（菜单/导航/快捷工具「日记」入口、日记本对话框（今日 + 历史 + 让思思现在写）、首开当日临时覆盖第一卡片槽并可恢复原配置）、`packages/ui/src/theme/workbench.css`（日记对话框/历史列表样式） | 2026-08-29 | `@aervox/diary` 单测 3（系统提示词含人写风格/用户提示词含「今日记忆」段/模板渲染含记忆计数）；`@aervox/api` `diary-rest.test.ts` 4（首次生成+默认每日计划懒建/同日幂等 existing 且不重复/rewrite=true 改写版本 version=2/历史列表）；`@aervox/worker` `diary-generator.test.ts` 2（定时生成真实模板内容含记忆、同日已存在跳过且游标推进）；既有 `diary-ondemand.test.ts` 5 与 `diary-cas.test.ts` 无回归；`mise tasks run ci-code`（含 check:boundary 新增 @aervox/diary 零违规）+ ci-docs | 原生 |
...
| 全量画像来源与动作改用户可选开关（胶囊开关复用 + 授权状态文字保留） | CAP-033 | `packages/ui/src/components/AervoxWorkbench.vue`（「全量画像来源与动作」列表：原「请求系统权限」/「撤销并删除」两个按钮替换为复用 `.settings-switch` 的胶囊开关，授权状态文字列保留；`:checked = osStatus==='granted'`，开启=`requestProactiveCapability`（守卫放宽：`user_revoked` 来源也可重新请求，清除撤销记录后由 Host 探测恢复 granted），关闭=`deleteProactiveSource` 撤销并删除本地数据（保留确认弹窗），`background.persistent` 关闭改走 `setProactivePersistence({background:false})` 对齐其授权事实源；操作后按服务端最新状态回写开关、确认取消/失败自然回滚；不可用态禁用并以 title 说明原因（待平台接入/随授权向导统一确认））、`packages/ui/src/theme/workbench.css`（`.settings-switch` 新增 `:disabled` 态与 `.proactive-capability-switch` flex 修饰；清理 `.proactive-request-button`/`.proactive-delete-button` 死样式与响应式引用，窄断点改 `.proactive-capability-actions` 跨行定位） | 2026-08-31 | `@aervox/ui` `vue-tsc` 通过、`vitest` 4/4；`mise tasks run ci-code` 全量（install 15 / build+typecheck 24 / test 22）全过 | 原生 |
| 桌面广播空窗口崩溃修复（proactive:desired-state 等主动智能 IPC 报 TypeError） | CAP-033 + CAP-001（主题同步） | `apps/desktop/src/main/index.ts`（`broadcastProactiveStatus`/`broadcastTheme` 的空值保护 `!window?.isDestroyed()` 写反：`window` 为 null 时 `window?.isDestroyed()` 为 `undefined`、`!undefined === true` 仍进入分支对 null 解引用 `.webContents` → 桌宠窗口未开时（`petWindow` 常态为 null，关闭后亦置 null）主动智能任何状态变更广播（authorize/desired-state/persistence/capability/source:delete 收尾均调用）都抛 `Cannot read properties of null (reading 'webContents')`，IPC invoke 整体失败、设置页全部操作报错；主题广播同型 bug 且以 `@ts-ignore` 压掉本可暴露问题的类型错误。修复：守卫改 `window && !window.isDestroyed()` 显式排除 null 并移除 `@ts-ignore`，与既有正确写法（emote 转发 `petWindow && !petWindow.isDestroyed()`）对齐） | 2026-08-31 | `@aervox/desktop` `vue-tsc` 通过、`vitest` 7 文件 30/30；`mise tasks run ci-code` 全量（install 15 / build+typecheck 24 / test 22）全过 | 原生 |
| 仓库根目录治理精简（IDE 过程文档移除 + workspace 预留意图注释 + 展示站副本出库） | 基础设施（治理） | 根 `.trae/` 整目录移除（8 份 IDE 过程计划文档：diary-ai-summary-refactor、offline-voice-input-asr、quiz-mode-implementation、refactor_study_and_mistake_modal、settings-plugin-skills-mcp、study-mode-card-switch、ui-adaptive-layout-fix、学习规划重构——其落地事实均已在本表 §4.2 登记闭环，docs/ 零引用、构建/门禁配置零引用，违反文档治理"权威内容一律深链 docs/"）；`pnpm-workspace.yaml`（`modules/*` 条目补预留意图注释：ADR-014 模块化宿主目录尚未创建，pnpm 对不存在通配目录无副作用，保留待迁移）；本地未跟踪的 `Aervox-Siyu-20260829_副本/` 静态展示站副本已移出仓库根（非 git 内容，仅清理工作区噪音） | 2026-09-01 | `mise tasks run ci-code` 全量（install 15 / build+typecheck 24 / test 22）与 `ci-docs` 0 errors 全过；`git status` 无 .trae 残留、根目录条目回归全跟踪/全忽略两态 | 原生 |
| 桌面端自动化打包流程 + 品牌图标进产物（CAP-001 发布基础设施） | CAP-001 + 基础设施（发布） | `apps/desktop/electron-builder.yml`（新增：`electron-vite` `out/` 产物 → dmg/zip（macOS arm64+x64）/nsis（win x64）/AppImage（linux x64）；`extraResources` 携带 `build/icon.ico` 供主进程打包态窗口图标（`process.resourcesPath/icon.ico`）使用；dev 无签名证书 macOS `identity: null` 跳过 codesign，正式分发前需接入 Developer ID + 公证）、`apps/desktop/scripts/generate-icons.mjs`（品牌 SVG 追加渲染 512/1024 高密度 PNG——icns 需 ≥512 源，避免上采样模糊）、`apps/desktop/package.json`（`dist`/`dist:pack`/`dist:dir` 脚本 + `author`）、根 `package.json`（`dist:desktop`：turbo 过滤构建 workspace 依赖后打包）、`pnpm-workspace.yaml`（`allowBuilds.electron-winstaller: false`——`electron-builder` 传递依赖仅 Squirrel 目标使用，本项目 NSIS 不需要其安装脚本）、`aervox`（`package` 子命令 + 帮助文案）、`.gitignore`（`apps/desktop/release/`）、`.github/workflows/package.yml`（三平台矩阵：推 `v*` 标签或手动触发，`pnpm/action-setup`+`setup-node` 24 与 mise 锁定对齐，turbo 缓存与 Electron 二进制缓存，`electron-builder --publish never` 后统一 `softprops/action-gh-release` 发 Release，规避内建 publisher 对 tag-版本一致的强约束；手动触发仅出 artifact） | 2026-08-31 | 本地 `dist:dir`（mac-arm64）：`Aervox.app` 生成、`CFBundleIconFile=icon.icns`、`Resources/icon.icns`（186KB 有效 icns）与 `icon.ico`（extraResources）落位、`CFBundleIdentifier=com.aervox.desktop`；启动冒烟：打包应用进程存活、无崩溃日志、osascript 优雅退出无残留；`mise tasks run ci-code` 全量（install 15 / build+typecheck 24 / test 22）全过；CI 工作流经 YAML 解析核验（Package / 三平台矩阵 / 10 步 / Release 步骤仅 tag 触发） | 原生 |
| 全栈性能体系多维优化（SQLite 运行时 PRAGMA + 列元数据缓存 + 静态契约预序列化/HTTP缓存 + Web/Desktop Chunk 分割与抽屉懒加载 + Live2D 后台休眠 + Markdown LRU 记忆化） | 基础设施（性能） + NFR-PERF-001 | `packages/database/src/client.ts`（追加 `cache_size` 64MB、`temp_store` MEMORY、`mmap_size` 256MB PRAGMA 调优）、`packages/database/src/schema/init.ts`（`initDatabaseSchema` 增加 scoped `tableColumnsCache` / `WeakMap` 列元数据缓存减少重复 PRAGMA 查询，单次建表耗时降低 ~65%，彻底消除测试并发超时）、`apps/api/src/app.ts`（`/openapi.json` 预序列化 + `Cache-Control: public, max-age=3600` 缓存头）、`apps/web/vite.config.ts` 与 `apps/desktop/electron.vite.config.ts`（配置 Rollup `manualChunks` 将 `pixi`/`element`/`icons`/`vue` 细粒度拆分，消除 Vite 500KB 超大 chunk 告警，Web 主入口 bundle 从 1.95MB 降至 281KB，缩减 85.5%）、`packages/ui/src/components/AervoxWorkbench.vue`（四大抽屉组件与 Live2DPet 转为 `defineAsyncComponent` 异步按需加载）、`packages/ui/src/utils/markdown.ts`（增加 500 上限的 LRU 记忆化缓存，消除高频重复解析）、`packages/ui/src/live2d/controller.ts`（接入 `visibilitychange` 监听，后台隐藏标签页自动停用 Pixi Ticker 消除无谓跑帧耗电）、`packages/ui/src/theme/workbench.css`（`.vn-history-line` 增加 `content-visibility: auto; contain-intrinsic-size: 0 42px;` 隔离视口外部渲染） | 2026-09-09 | `@aervox/database` 37 套测试全部通过（`test/llm-config.test.ts` 超时消除，运行时间从 >10s 降至 1.6s）；`packages/ui` 测试（新增 `markdown-cache.test.ts`）全部通过；`apps/api` `openapi-contract` 测试通过；Web 与 Desktop 构建全部成功并验证 chunk 拆解 | 原生 |
| 插件系统与工作台综合质量闭环治理（内置插件合并 + 专注模式工作台零硬编码解耦与通用插件运行时 + 消息变换管道 + 专注模式 Prompt 模块彻底解耦与 extraSections 扩展机制 + 刷题模式统一并入专注模式 focus-mode 闭环能力 + Base Prompt 彻底剥离模式内容保持纯净通用底座 + 服务端 Turn 插件生命周期管道 ServerTurnPlugin + 全端结构化元数据 metadata 传递消除文本污染 + 提示词人设协同优化 + SQLite DDL/列缓存安全加固 + UI 抽屉懒加载与组件库按需瘦身 + 插件开发操作指南与规范全流程完善（Bundle声明+ServerTurnPlugin切面注入+结构化metadata+UI扩展） + UI 真实 SFC 单测覆盖 + MCP 预设变更冒泡修复） | CAP-001/002/003/007/016/020 + 基础设施 | `plugins/focus-mode/`（`plugin.manifest.json`、`config.schema.json`、`SKILL.md`）、`packages/contracts/src/schemas.ts`、`packages/api-client/src/{transport,desktop-transport,useAervoxTurn}.ts`、`apps/desktop/src/{preload/domains/aervox-api.ts,main/index.ts,renderer/src/env.d.ts}`、`apps/api/src/modules/plugins/{index,config-service}.ts`、`apps/api/src/modules/plugins/turn-plugins/{types,registry,runner,focus-mode,study-mode,quiz-mode,index}.ts`、`apps/api/src/modules/conversation/{agent-executor,routes}.ts`、`packages/practice-review/src/terms.ts`、`packages/agent-loop/src/{base-prompt,focus-mode-prompt,study-mode-prompt,index}.ts`、`packages/database/src/schema/init.ts`、`packages/ui/src/{components/AervoxWorkbench.vue,components/workbench/drawers/SettingsModal.vue,components/plugin/McpToolsTab.vue,composables/useWorkbenchLayout.ts,index.ts,registry/{types,ui-registry}.ts,plugins/{index,plugin-runtime,focus-mode/*,study-mode/index}.ts,utils/element.ts}`、`apps/web/{src/main.ts,vite.config.ts}`、`apps/desktop/{src/renderer/src/main.ts,electron.vite.config.ts}`、`packages/ui/vitest.config.ts`、`docs/how-to/develop-plugin-ui-extension.md`、`docs/reference/{plugin-config-and-pages,DATABASE}.md`、`apps/api/test/{study-term-plugins,quiz-mode,conversation-loop,openapi-contract}.test.ts`、`packages/agent-loop/test/context-builder.test.ts`、`packages/database/test/schema-init-safety.test.ts`、`packages/ui/test/{components-sfc,study-mode-plugin}.test.ts` | 2026-09-10 | API 单元与集成测试（focus-mode 与 quiz 意图统一闭环验证通过，全端结构化 metadata 纯净消息触发通过，ServerTurnPlugin 管道解耦通过，study-mode 兼容别名验证通过）；Agent Loop 单元测试（166/166 全部通过，验证 base-prompt 纯净底座与 extraSections 注入机制）；Contracts 契约与 OpenAPI 测试（5/5 通过）；Database 仓储测试（195/195 通过）；UI 测试（31/31 全部通过）；全端构建与类型检查通过；ci-docs 门禁零错误 | 原生 |
| CR-030 纯本地数据库与系统一致性加固（历史租户唯一索引无损平移迁移 + 幂等去重修复 + 纯本地 SQLite 约束收敛 + UI 插件运行时激活状态与清理函数解耦 + 仓储层无残留条件清理） | CR-030 + 基础设施（持久化与插件系统） | `packages/database/src/client.ts`（彻底移除 authToken 与远端 URL 兼容，纯本地与 :memory: 唯一支持，PRAGMA 无条件执行）、`packages/database/src/schema/init.ts`（新增 `migrateTenantUniqueIndexes` 迁移与 `cr-030.migrate-tenant-unique-indexes` Journal 记录，无损迁移 8 张表的租户复合唯一索引为全局唯一索引，并在重命名/替换前通过 `__dup_<id>` 规避历史冲突）、`packages/database/src/schema/*.ts`（20 份 Drizzle 表定义唯一索引名对齐）、`packages/database/src/tenant.ts`（TenantContext 标记 deprecated 纯本地兼容类型）、`packages/database/src/repositories/sqlite/*.ts`（17 份仓储清理无用空 `where(and())` 与尾部空白）、`packages/ui/src/plugins/plugin-runtime.ts`（解耦 activePlugins 激活态与 activeCleanups 清理函数映射，消除无清理函数的插件在 sync 时重复 setup 的缺陷）、`docs/reference/DATABASE.md`、`docs/explanation/database-split-plan.md` | 2026-09-10 | `@aervox/database` 37 套测试 190 用例全绿（新增 `schema-init-safety.test.ts` 复合索引升级与重名冲突平移用例、`proactive-vault-client.test.ts` 纯本地约束测试）；`@aervox/ui` 插件运行时与专注模式测试（7/7）全绿；`git diff --check` 0 尾部空白；`ci-docs` 门禁全通 | 原生 |
...
| Agent Harness Loop 阶段 6f：AERVOX_LOOP_DRIVER=dsh API 接线（整 Turn 进程外 Adapter 组合根可开关） | CAP-002/007/020/027 + 基础设施（ADR-010） | `packages/config`（`ApiLoopDriver` native\|dsh + `AERVOX_LOOP_DRIVER` 启动期枚举校验，pi 保留 fail-fast）、`apps/api/src/modules/conversation/dsh-adapter.ts`（`resolveDshTurnAdapter` lazy 单例：缓存已准入 stdio handle 与 probe 禁用态；repoRoot 取 `AERVOX_DSH_REPO_ROOT` 或 cwd 向上查找）、`apps/api/src/modules/conversation/agent-executor.ts`（`runLoopTurnOnce` dsh 分支 → `runAdapterTurn` 整 Turn 执行，Provider/工具/上下文跳过；终态对齐 turns 表 Completed/Interrupted/Failed，skipped 不覆盖；准入失败 fail-closed `ADAPTER_UNAVAILABLE`+Failed 不静默回退 native；terms 抽取抽为 `extractStudyTerms` 双路径复用）、`apps/api/test/conversation-dsh.test.ts`、`packages/config/test/config.test.ts`、`docs/reference/adr/ADR-010-dsh-pi-adapters.md`（实施进展 6f）+ `agent-harness-loop.md`（§16.29 + §3 现状注记） | 2026-08-31 | `conversation-dsh.test.ts` 4（准入失败 fail-closed 不回退 native / 禁用态缓存快速失败 / resolver `submodule_missing` / `it.runIf` 子模块就绪+本地兼容端点整 Turn message→delta→done Completed）；`@aervox/config` 6（loopDriver 缺省/覆盖/pi·bogus fail-fast）；`@aervox/api` 297（diary-ondemand 既有缺陷经同分支 todayWindow 修复后全量通过）；ci-code 全量 | `DSH-01`（固定 commit；runner 仍为协议+真实模型回合骨架，库内 Cordis 容器组装为 P2） |
| 基础设施加固：日记「当日素材」窗口时区缺陷（CAP-009 缺陷：本地 0 点至 UTC 零点间 materialCount=0） | CAP-009 + 基础设施 | `packages/diary/src/generation.ts`（`todayWindow` 本地零点经 `Date` 换算为规范 UTC ISO，与 `message_versions.createdAt` 的 `toISOString()` 存储格式对齐；不再用无时区本地串「近似」导致东半球时区本地 0-offset 点位当天消息被字符串比较排除）、`packages/diary/test/diary-core.test.ts`（todayWindow 规范 UTC 格式 + 回换本地零点 + 日期标签一致不变量回归） | 2026-08-31 | `diary-core.test.ts` 5（新增 todayWindow 1）；`apps/api` diary-ondemand 5（此前本地 UTC+8 0-8 点稳定失败 materialCount=0，HEAD stash 复现确认既有缺陷，修复后通过）；ci-code 全量 | 原生 |
...
| W-12 proactive 适配器公开面收敛 + 类型漂移消除（REFACTOR-PLAN W-12） | CAP-033 | `apps/desktop/src/main/proactive-source-adapters.ts`（删除死别名 `createElectronProactiveSourceAdapters`、`createProactiveCapabilityProbe` 改模块私有——全仓库 preload/renderer/dist 零引用，28 个导出收敛为 6 个有外部引用；`createProactiveSourceAdapters`/`toCapabilityProbe`/`ProactiveWideSourceId` 保留供 index.ts 与测试使用）、`apps/desktop/src/main/proactive-host.ts`（删除本地自写 3 字段 `ProactiveCapabilityProbeResult` 定义，改从 `./proactive-source-adapters` 导入 6 字段完整版——消除类型漂移：此前 index.ts 用 adapters 的 `toCapabilityProbe()`（6 字段）填充 host `capabilityProbe` 回调（仅认 3 字段），完整数据在 host 侧被类型系统截断） | 2026-09-04 | `@aervox/desktop` `vue-tsc --noEmit` 0 错误；Vitest 7 文件 30/30 通过；`mise tasks run ci-code` 待 PR 全量复核 | 原生 |
...
| 工作台 UI 全面组件化与插件插槽注入/组件替换体系 | CAP-001/020 / ADR-009 / ADR-015 | `packages/ui/src/components/{AervoxWorkbench.vue,extension/ExtensionSlot.vue,workbench/*}`、`packages/ui/src/composables/*`、`packages/ui/src/registry/*`、`docs/reference/plugin-config-and-pages.md`、`docs/how-to/develop-plugin-ui-extension.md`、`refactor-plan.md` | 2026-09-09 | `@aervox/ui` 单元测试 19/19 全绿（`ui-registry.test.ts`、`workbench-composables.test.ts` 等）；`@aervox/ui` 与 `@aervox/desktop` 类型检查零错误；`@aervox/web` 生产构建通过 | 原生 |
| 专注模式与术语下钻第一方插件（AVX-PLUG-001 第一方插件） | CAP-007/020 / ADR-009 / ADR-015 | `packages/ui/src/plugins/study-mode/`（`StudyModeSwitch.vue`、`StudyTermsBar.vue`、`TermExploreDialog.vue`、`index.ts`）、`packages/ui/src/components/{AervoxWorkbench.vue,workbench/WorkbenchHeader.vue,workbench/ConversationConsole.vue,workbench/TermsBar.vue,TermExploreDialog.vue}`、`packages/ui/src/registry/index.ts`、`packages/ui/test/study-mode-plugin.test.ts` | 2026-09-09 | `@aervox/ui` 单元测试 22/22 全绿（含 `study-mode-plugin.test.ts` 插槽注册/优先级/注销/幂等性测试）；`@aervox/ui`、`@aervox/web`、`@aervox/desktop` `vue-tsc` 检查零错误；向后兼容导出校验通过 | 原生 + `AVX-PLUG-001` 规范 |
