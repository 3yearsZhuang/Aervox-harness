...
| CAP-008 情绪价值与安全陪伴边界防御（Crisis Safety Gate & Emotional Companionship）：定义 `@aervox/contracts` 安全模式与 OpenAPI 路由；实现确定性纯函数安全分类器 `classifySafety`（分级 crisis_high / distress_moderate / safe_normal，覆盖自残自杀/暴力危害/越狱注入/情绪困扰，内置日常口语误报抑制）；提供权威地区求助热线目录与不可篡改危机应答；`apps/api` 注册 `safety` 领域模块并暴露 `GET /v1/safety/resources` 与 `GET /v1/safety/incidents`；`agent-executor` 接入前置安全门禁（高危自杀即刻阻断大模型与工具，直推固定求助热线 SSE 流，消息标记 isRedacted=1 确保日记素材与长期记忆零泄漏，记录安全审计；中度困扰温和注入共情陪伴指引至 extraSections，禁病理诊断与说教，不可被 Persona 设定覆盖） | CAP-008 | `packages/contracts/src/safety-schemas.ts`、`packages/contracts/src/openapi.ts`、`apps/api/src/modules/safety/`、`apps/api/src/modules/context.ts`、`apps/api/src/app.ts`、`apps/api/src/modules/conversation/agent-executor.ts`、`apps/api/src/modules/conversation/routes.ts`、`packages/repositories/src/repositories/sqlite/conversation-repository.ts`、`packages/observability/src/metric-names.ts`、`apps/api/test/safety-companion.test.ts` | 2026-09-13 | 12 项安全伴随测试（AC-FR-SAFE-001-01~03 / TC-SEC-SAFE-001）100% 通过；`apps/api` 全量 366/366 单元与集成测试通过；`node scripts/import-boundary.mjs` 零违规；日记素材收集器零泄漏实测通过 | 原生 |
| 全链路可观测性与结构化日志系统落地（AVX-HAR-001 §16.3 / OPS-REL-001）：补齐 `@aervox/observability` 零依赖生产实现（`StandardLogger` 支持 json/pretty、等级过滤与子上下文；`InMemoryMetricsRegistry` 支持 counters/gauges/histograms 分布聚合与 Prometheus OpenMetrics 导出）；`@aervox/config` 支持 `AERVOX_LOG_LEVEL` 与 `AERVOX_LOG_FORMAT`；`apps/api` 接入 HTTP 访问日志、统一错误结构化捕获、`GET /v1/metrics` 监控导出端点与 Agent Turn/Tool 生命周期指标接线；`apps/worker` 接入类型化日志器；补充根目录 `.env.example` 完整环境变量配置基准模板 | 基础设施（可观测性） | `packages/observability/`、`packages/config/`、`apps/api/src/app.ts`、`apps/api/src/modules/conversation/agent-executor.ts`、`apps/api/test/observability.test.ts`、`apps/worker/src/index.ts`、`.env.example` | 2026-09-13 | `packages/observability` 单测 16/16 通过；`packages/config` 单测 6/6 通过；`apps/worker` 单测 10/10 通过；`apps/api` 单测与集成测试 354/354 通过（含 `observability.test.ts` 5 项端到端指标与结构化日志断言）；`node scripts/import-boundary.mjs` 零违规通过 | 原生 |
...
| 基础设施加固：租户认证中间件与已验证租户上下文（缺陷1，历史实现） | 基础设施（API 认证/租户隔离） | 历史 `apps/api/src/shared/{auth,tenant}.ts`、`apps/api/src/app.ts`、`apps/api/test/auth.test.ts`；租户入口已由 CR-030 删除 | 2026-08-28 | 历史 `auth.test.ts` 集成测试 4；当前替代证据见 CR-030 D3 本机 API 边界登记 | 原生 |
...
| 基础设施加固：token×租户绑定 + fail-closed（缺陷A·批次2，历史实现） | 基础设施（API 认证/租户隔离；TM-001/TM-004） | 历史 `apps/api/src/shared/auth.ts`；`AERVOX_AUTH_WORKSPACE/USER` 与租户绑定已由 CR-030 删除，保留 Bearer token 与可选审计 actor | 2026-08-28 | 历史 token 绑定测试；当前替代证据见 CR-030 D3 本机 API 边界登记 | 原生 |
...
| W-19 阶段 5：持久层切消费面（4 消费包 import 与依赖彻底归零） | 基础设施（W-19 包拆分） | `packages/diary/`、`packages/host-agent/`、`apps/worker/`、`apps/api/` 的 `package.json`（移除 `@aervox/database`，引入 `@aervox/schema` 与 `@aervox/repositories`）及 130+ 源码/测试文件（`from "@aervox/database"` 收窄重定向）；`pnpm-lock.yaml` | 2026-09-10 | `check:boundary` 14/14 全绿；全仓 19 包 `build` 与 `typecheck` 0 错误；`diary` 5/5、`host-agent` 67/67、`worker` 10/10、`api` 322/322、`repositories` 192/192 测试全绿；全仓除兼容包自身外 `git grep 'from "@aervox/database"'` 源码与测试彻底归零，为阶段 6 彻底删除兼容包达成退出前置条件 | 原生 |
| W-19 阶段 6：清理兼容包（持久层双包最终态落地与旧包退出） | 基础设施（W-19 包拆分） | 彻底删除 `packages/database/` 目录（`src/index.ts`、`package.json`、`tsconfig.json`）；`pnpm-lock.yaml`（剪除 `packages/database`，全仓收敛至 18 个工作区包）；`pnpm-workspace.yaml`（更新注释）；`scripts/import-boundary.{mjs,test.mjs}`（规则升级禁止 `(database|schema|repositories)` 触库）；`README.md`、`AGENTS.md`、`database-split-plan.md` | 2026-09-10 | `check:boundary` 14/14 全绿；全仓 18 包 `turbo run build` 与 `turbo run typecheck` 0 报错；`repositories` 192/192、`api` 322/322、`worker` 10/10、`host-agent` 67/67 测试全绿；全仓除历史文档外源码/配置 `@aervox/database` 0 残留；持久层 `@aervox/schema` + `@aervox/repositories` 双包最终态架构落地，W-19 重构全阶段圆满结项 | 原生 |
...
| CR-030 D0：SQLite 永久本地单用户真源与去租户化决策基线 | CAP-005/015/026/027/033～035、SEC-LOC-001、DATA-PRO-001、NFR-SEC/PRIV/DR-001 | `docs/reference/changes/CR-030-pure-local-sqlite-database.md`；同步 `PRD.md`、`prd-cap-acceptance.md`、`SRS.md`、`srs-proactive-intelligence.md`、`ARCHITECTURE.md`、`DATABASE.md`、`database-coverage-matrix.md`、`DATA_PRIVACY.md`、`AI_QUALITY_SAFETY.md`、`THREAT_MODEL.md`、`TEST_STRATEGY.md`、`operations.md`、ADR-003/008、CR-003 与导航/登记 | 2026-09-11 | `mise tasks run ci-docs`；CR-030 decision=accepted、delivery=implemented；D1～D3 实现证据已拆分登记；`SEC-TEN-001` 已标记 Deprecated 并由 `SEC-LOC-001` 替代 | 原生 |
| CR-030 D1 基础切片：迁移 sidecar 状态机、启动中间态 fail-closed、旧租户范围扫描与显式选择 | CR-030 / SEC-LOC-001 / NFR-REL-001 | `packages/repositories/src/migration/cr030-migration.ts`、`packages/repositories/src/migration/index.ts`、`packages/repositories/test/cr030-migration.test.ts` | 2026-09-11 | `mise x -- pnpm --filter @aervox/schema build`；`mise x -- pnpm --filter @aervox/repositories typecheck`；CR-030 定向 Vitest 4 tests；状态机、启动 fail-closed、范围扫描和显式选择已实现 | 原生 |
| CR-030 D1 备份清单切片：业务库/Vault/RecoveryControlLedger/WAL artifact 复制、SHA-256 manifest 与篡改检测 | CR-030 / NFR-DR-001 / NFR-REL-001 | `packages/repositories/src/migration/backup-manifest.ts`、`packages/repositories/src/migration/index.ts`、`packages/repositories/test/backup-manifest.test.ts` | 2026-09-11 | CR-030 定向 Vitest 2 tests；`@aervox/repositories` typecheck；artifact 复制、SHA-256 manifest 和篡改检测已实现，停写协调由运行编排负责 | 原生 |
| CR-030 D1 staging/换库切片：外键拓扑复制、选定范围过滤、完整性/外键/行数校验与同文件系统原子替换 | CR-030 / NFR-DR-001 / NFR-REL-001 | `packages/repositories/src/migration/staging-migration.ts`、`packages/repositories/src/migration/index.ts`、`packages/repositories/test/staging-migration.test.ts` | 2026-09-11 | CR-030 定向 Vitest 2 tests；`@aervox/repositories` typecheck；staging、完整性/外键/行数校验和同文件系统原子换库已实现，生产 FTS 重建与 rollback 保留需发布前演练 | 原生 |
| CR-030 D1 API 入口守卫：默认 loopback，open 认证禁止非 loopback，token 认证允许显式远程监听 | CR-030 / SEC-LOC-001 | `packages/config/src/index.ts`、`apps/api/src/index.ts`、`apps/api/src/shared/auth.ts`、`packages/config/test/config.test.ts`、`apps/api/test/auth.test.ts` | 2026-09-11 | config/auth 定向测试；远端 CI build/typecheck；API 入口启动期 fail-closed | 原生 |
