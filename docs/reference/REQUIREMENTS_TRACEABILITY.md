...
| 插件系统与工作台综合质量闭环治理（内置插件合并 + 专注模式工作台零硬编码解耦与通用插件运行时 + 消息变换管道 + SQLite DDL/列缓存安全加固 + UI 抽屉懒加载与组件库按需瘦身 + 插件文档/契约双签名对齐 + UI 真实 SFC 单测覆盖 + MCP 预设变更冒泡修复） | CAP-001/002/007/020 + 基础设施 | `apps/api/src/modules/plugins/{index,config-service}.ts`、`apps/api/src/modules/conversation/agent-executor.ts`、`packages/practice-review/src/terms.ts`、`packages/agent-loop/src/base-prompt.ts`、`packages/repositories/src/schema/init.ts`、`packages/ui/src/{components/AervoxWorkbench.vue,components/workbench/drawers/SettingsModal.vue,components/plugin/McpToolsTab.vue,composables/useWorkbenchLayout.ts,index.ts,registry/{types,ui-registry}.ts,plugins/{index,plugin-runtime,study-mode/index}.ts,utils/element.ts}`、`apps/web/{src/main.ts,vite.config.ts}`、`apps/desktop/{src/renderer/src/main.ts,electron.vite.config.ts}`、`packages/ui/vitest.config.ts`、`docs/how-to/develop-plugin-ui-extension.md`、`docs/reference/{plugin-config-and-pages,DATABASE}.md`、`apps/api/test/study-term-plugins.test.ts`、`packages/repositories/test/schema-init-safety.test.ts`、`packages/ui/test/{components-sfc,study-mode-plugin}.test.ts` | 2026-09-10 | API 单元与集成测试（study-mode 预装与服务端启用门控用例通过）；Database 仓储测试（195/195 通过，验证 WeakMap 列缓存隔离与 DDL 异常抛出）；UI 测试（覆盖消息变换管道、零硬编码插件生命周期与配置同步 5/5 通过，真实 SFC 挂载 3/3 通过）；`apps/web` 与 `apps/desktop` 构建与类型检查成功；ci-docs 门禁零错误 | 原生 |
