...
| 流事件序号并发分配修复（UQ-01 sequence 冲突） | CAP-002/009 | `packages/database/src/repositories/{types,sqlite/conversation-repository}.ts`（appendStreamEvent 原子分配 MAX+1 + 冲突回退）、`apps/api/src/modules/conversation/{user-question-coordinator,routes}.ts`（去掉读条数+1） | 2026-08-29 | `turn-stream-sequence.test.ts` 回归 3 项（原子分配/冲突回退/非序号冲突原样抛出）；执行器心跳续租 + 提问超时防循环；全仓 typecheck/test；真实 DeepSeek 提问回合验证（Completed、无错误事件、序号唯一） | 原生 |
