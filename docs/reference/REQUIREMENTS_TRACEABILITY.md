...
| Agent Harness Loop 阶段 3c+-B2：长调用周期心跳续租 | CAP-002/007 + 基础设施 | `packages/agent-loop`（`lease-heartbeat.ts` 新增 `LeaseHeartbeat`：setInterval 周期续租 + CAS 丢失判定（renew ok=false）+ lost 订阅回调 + `throwIfLost` 检查点 + 幂等单播；`executor.ts` `ExecuteTurnOptions` 增 `leaseTtlMs`（默认 60_000）与 `leaseHeartbeatIntervalMs`（默认 TTL/2，0 关闭）；claim 后启动、try/finally 兜底停止；model stream chunk 间 `throwIfLost`，长工具调用 `onLost→cancel.abort()` + 工具 catch 内 `heartbeat.lost` 收口 `lease_lost`；`index.ts` 导出）；宿主无改动（复用既有 `renewAttemptLease` CAS） | 2026-08-28 | `@aervox/agent-loop` 126（新增 `lease-heartbeat.test.ts` 5：单元 lost 判定/幂等多播/stop 后停更 + 集成——120ms 长工具调用期间续租 ≥3（防租约超时被恢复器误判）且 Turn 正常完成 / 长调用中途 simulatePreemption（fencing+1）→ 心跳续租失败 → 在途工具 abort → failed(lease_lost) 且无 tool_result/done/error 迟到事件、不写终态 / `leaseHeartbeatIntervalMs=0` 时仅 Step 首部探活）；`@aervox/database` 150、`@aervox/host-agent` 62、`@aervox/api` 229 无回归；`mise tasks run ci-code` 全量（17 tasks）+ check:boundary 零违规；ci-docs | 原生 |
