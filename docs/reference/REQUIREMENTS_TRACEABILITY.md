...
| `CAP-009` | AI 每日日记 | `P0 · R1.5` | `Specified` | Not Ready | ✔ | [AI 每日日记](PRD.md#prd-cap-009)、[日记与记忆层的关系](PRD.md#prd-diary-memory)、[CR-024](changes/CR-024-on-demand-diary.md) | 对话触发路径已落地（CR-024 §4.2）；定时任务幂等、重试、版本冲突、来源快照、通知和时区边界测试仍待阶段 2 |
...
| 学习模式切换开关样式修复（悬浮顶栏） | CAP-001 | `packages/ui/src/theme/workbench.css`（上游 #96 引入的 `.floating-top-actions` 顶栏容器与 `.floating-study-switch-wrap` 学习模式开关模板无任何配套 CSS 导致按钮裸奔不可用——补齐：顶栏容器 absolute 右上角 flex 横排取代原 `.floating-settings` 独立定位；开关胶囊玻璃材质（图标 + 「学习模式」文案 + iOS 风格 34×19 轨道 + 15px 圆滑块），开启态轨道 accent 雾蓝 + 滑块 translateX(15px)、wrap 加 `on` class 变 accent；≤620px 断点隐藏文案只留图标 + 轨道防与左侧菜单胶囊拥挤）、`packages/ui/src/components/AervoxWorkbench.vue`（label 补 `:class="{on: studyModeEnabled}"` 显式绑定） | 2026-08-29 | 浏览器实测：开关胶囊高 34px 与设置按钮同行（top 15/12 垂直居中差 < 4px）、右侧间距 8px；点击开启轨道变 rgb(78,119,209) accent、滑块 matrix(1,0,0,1,15,0) 位移、收起态输入框「学习模式」chip 联动出现；再点击关闭恢复灰底；492px 窄视口文案正确隐藏；`@aervox/ui` typecheck/build 通过、ci-docs 0 errors | 原生 |
...
| 视觉小说式分句呈现（对话框限高 + 首句先行 + 下一句按钮） | CAP-001 | `packages/ui/src/components/AervoxWorkbench.vue`（`splitIntoSentences` 按 `(?<=[。!?!?…])\s*|\n+` 切句保留句末标点；`novelReleased` 释放计数 + `novelSentences`/`novelStreamingText`（流式仅显示第一句，打字机效果落在首句、余句静默缓存）/`novelDisplayText`（已释放句子 join）/`queuedSentenceCount`/`hasQueuedSentence` computed；`watch(latestAssistantLine)` 新回复重置释放进度；`revealNextSentence` 释放下一句并 `petReact({speak})` 桌宠同步念出；模板 streaming 分支渲染首句、complete 分支渲染已释放句 + `.novel-next-btn` 按钮（`:key="novelReleased"` 触发新句重渲动画）；对话框高度沿用既有 `max-height: clamp(150px, 34vh, 460px)` 约束，分句后单句内容自然受限）、`packages/ui/src/theme/workbench.css`（`.message-novel-text .markdown-body` `.28s` 新句淡入；`.novel-next-btn` 右对齐玻璃胶囊（accent 38% 边框/10% 底 + hover 18% 抬升）、`.novel-next-count` 剩余句数徽标（tabular-nums）、`novel-next-in` `.34s` 上浮入场；对话框最大高度收紧：主 clamp(120px, 26vh, 320px)（原 34vh/460px）、窄断点 clamp(160px, 40vh, 380px)、矮屏断点 38vh；「下一句」按钮左对齐贴文本（justify-self: start、margin-top 6px、padding 4px 12px、徽标 16px）不占额外高度）。**分句呈现回归修复**：`handleSend` 中 `assistantLine` 为 push 前创建的原始对象，`onDelta`/`onDone`/catch 对其赋值绕过 reactive 代理 set trap——旧模板直接渲染 `.text` 时靠组件级重渲染兜底可显示，新增 `novelSentences` 等 computed 依赖代理 `.text`/`.state` 后依赖永不失效、computed 永不重算，导致流式无输出且兜底/错误文案均不显示（一直「正在连接」）。修复：push 后从 reactive 数组取回代理 `liveAssistantLine = story.value[story.value.length - 1]` 并全部改用之赋值；catch 补 `console.error` 便于诊断 | 2026-08-29 | `@aervox/ui` typecheck（`vue-tsc` = build）+ `vitest` 3/3 通过；浏览器实测（5173）：无桥环境下错误文案「连接失败：fairyDesktop 桥不可用…」即时上屏（修复前同一场景显示空/「正在连接」，证明赋值→UI 链路恢复）；桌面端 dev 冒烟建议：长回复流式期间仅首句打字机显示、完成后左下角出现紧凑「下一句 N」按钮、逐句释放带淡入、全部释放后显示完整文本 | 原生 |
