...
| CAP-030 | 主动提醒深化 | 运行 | 频率上限、免打扰、一键关闭；CR-032 落地全局防打扰裁决器（冷却/静音时段/每小时频次水位）与托盘一键熔断 |
| CAP-031 | 内容/技能市场 | 双 | 需版权、支付、退款、平台治理与法务 |
| CAP-032 | 机构/监护模式 | 运行 | 需字段级权限与监护人流程 |
| CAP-033 | 全域感知与个人画像（主动智能模式） | 双 | 用户独立确认 `full_profile_v1` 后启用；桌面 Host 构建能力 + 运行时来源/动作授权；后台生命周期、全量来源、七天原始副本提炼、本地存储与导出均受 CAP-033 契约约束；CR-032 起感知源目录含 `system.idle_state`，主动动作支持插件派发 |
| CAP-034 | Home Assistant 家庭环境连接 | 运行 | CAP-033 active 且 `device.sensors` 有效后配置；本地 REST/WS 网关、实体默认禁用、service 白名单与 `action.external` 审计 |
| CAP-035 | 运动健康信号连接 | 运行 | CAP-033 active 且 `restricted.profile` 有效后配置；用户自有官方开放平台凭据、本地每日指标、只读工具与连接级撤销删除 |
