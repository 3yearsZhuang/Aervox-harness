...
- **必须**：默认题组从当前租户的活动题目中按创建顺序返回，默认 3 题且可请求 3～5 题；每题提交后返回正确/错误/待确认和下一步（分别为 `continue`、`review_scheduled`、`await_review`）；服务端根据题目的标准答案判定可确定答案（标准化比较，忽略大小写与首尾空白），短文本题不可验证时进入待确认。
