...
## 7. 产品上限增强候选需求（A/B 档评估）

本节收录"养成系叙事陪伴"产品评估中采纳的候选需求。全部挂靠现有 CAP，不新增能力地图条目；未进入对应阶段前保持 `Specified`，进入开发批次时按 §6 补齐证据并通过 DoR。B 档带 `（拆步）` 的需求拆分主行为与依赖项，依赖项落在 `CAP-030` 主动提醒与 `CAP-018` 桌面表现层。

### FR-MEM-001 记忆与日记语义检索（潜意识检索）

- **Parent CAP**：`CAP-005`、`CAP-027`
- **触发**：用户以自然语言查询过去记录（"我当时是怎么理解 X 的"）。
- **必须**：对已获授权的记忆记录与日记段落执行向量检索（复用 `memory_embeddings` 与混合检索），返回命中片段、来源与日期标签；仅召回仍在用户历史保留期与 AI 召回窗口内的内容；结果标记 `source`（记忆/日记）与置信度；不可用于隐私删除或撤权后仍可召回的绕过路径。
- **异常**：无 embedding、查询无命中、来源已删除/撤销、本地向量库不可用。
- **数据**：命中片段列表仅含元数据与允许展示的正文；删除传播同步清除索引。
- **验收**：
  - `AC-FR-MEM-001-01`：Given 用户查询过去对话中的概念，When 检索，Then 返回匹配片段、来源类型与日期，且全部在授权保留范围内。
  - `AC-FR-MEM-001-02`：Given 命中内容来源已删除或撤权，When 检索，Then 不返回该正文也不泄露存在性。
  - `AC-FR-MEM-001-03`：Given 无 embedding 或向量库失败，When 查询，Then 降级为关键词检索或明确提示，不伪装成功。
- **测试**：`TC-INTEG-MEM-001`、`TC-PRIV-MEM-001`。

### FR-DIA-002 第三人称传记视角

- **Parent CAP**：`CAP-009`
- **设置**：在既有 `视角` 枚举（桌宠视角/客观学习日报/用户第一人称草稿）中新增 `观察者传记` 选项；默认不启用。
- **必须**：以观察者视角撰写日记，回顾用户当日可追溯行为（发问、作答、复习、编辑、情绪）而非虚构心理；观察者叙事必须逐段可回溯到来源；不得编造未发生的动作、言论或动机；与用户第一人称模式同样标注 AI 生成，不得冒充真实第三方记录。
- **异常**：素材不足、来源被删除、视角与既有内容重复生成。
- **数据**：沿用 `DiaryVersion.perspective` 枚举扩展；不新增记忆晋升路径。
- **验收**：
  - `AC-FR-DIA-002-01`：Given 用户选择观察者传记视角，When 生成，Then 日记以第三人称叙述且每段可定位到当日来源。
  - `AC-FR-DIA-002-02`：Given 某行为未被授权或无法回溯，When 生成，Then 不叙述该行为、不虚构动机。
  - `AC-FR-DIA-002-03`：Given 用户未启用该视角，When 生成，Then 保持其默认视角不变化。
- **测试**：`TC-AIEVAL-DIA-001`、`TC-PRIV-DIA-001`。

### FR-KNO-001 灵感闪卡（反常识观点）

- **Parent CAP**：`CAP-015`
- **触发**：用户在概念/记忆节点查看卡片。
- **必须**：卡片正面为知识点，翻面展示 Agent 生成的"反常识观点"（抬杠式思辨）；观点必须标记为生成推断、可争议，不得作为正式学习事实或进入掌握度；支持双向链接到相关卡片与记忆节点；用户可标记观点质量进入反馈队列。
- **异常**：无相关卡片、生成失败、subjective 内容争议上报。
- **数据**：闪卡视图由记忆/思维宇宙节点派生，不新增业务真源。
- **验收**：
  - `AC-FR-KNO-001-01`：Given 用户翻开卡片，When 展示，Then 正面知识点与背面推断观点均可辨识且标注生成。
  - `AC-FR-KNO-001-02`：Given 用户点击关联节点，When 跳转，Then 可达双向链接的对应记忆树节点。
  - `AC-FR-KNO-001-03`：Given 模型生成观点，When 落库，Then 仅作为候选推断，不进入正式知识事实。
- **测试**：`TC-INTEG-KNO-001`、`TC-AIEVAL-KNO-001`。

### FR-MEM-002 时间胶囊（旧困惑召回）

- **Parent CAP**：`CAP-005`
- **触发**：用户输入或召回涉及历史相似概念时。
- **必须**：Agent 对比历史记忆命中"过往类似困惑/问题"，以时间胶囊形式提示（如"某天你也困惑过类似概念"）并给出当时上下文；仅回放可验证的历史记录，不虚构"当年的你"；可关闭该提醒。
- **异常**：无历史命中、历史来源被删、用户禁用。
- **数据**：时间胶囊为检索结果聚合视图，不新增常数表。
- **验收**：
  - `AC-FR-MEM-002-01`：Given 存在相似历史困惑，When 触发，Then 展示当时的困惑、上下文与日期，且可回溯来源。
  - `AC-FR-MEM-002-02`：Given 用户关闭时间胶囊提醒，When 后续触发点到来，Then 不再显示。
  - `AC-FR-MEM-002-03`：Given 历史来源已删除，When 触发，Then 不显示片段正文、不占用重复召回。
- **测试**：`TC-INTEG-MEM-002`、`TC-PRIV-MEM-001`。

### FR-DIA-003 梦境碎片记录与小说化（拆步 1）

- **Parent CAP**：`CAP-009`
- **触发**：用户以语音/文字记录梦境碎片。
- **必须**：保存用户原始碎片为日记素材；Agent 生成"超现实主义小说"式补全作为**独立可识别的 AI 创作段落**存入日记，与事实记录明确分隔；用户可编辑、删除、关闭；补全内容不进入记忆晋升；语音输入走既有多模态素材通道（CAP-012 依赖，未到阶段可先仅文字）。
- **异常**：碎片为空、生成失败、用户撤回授权。
- **数据**：日记段落来源标记 `dream_log`；素材保留受删除传播约束（`DATA-DEL-001`）。
- **验收**：
  - `AC-FR-DIA-003-01`：Given 用户提交梦境碎片，When 生成，Then 原始碎片与 AI 补全段落分开保存且明显标识"创作"。
  - `AC-FR-DIA-003-02`：Given 用户删除碎片，When 清理，Then 关联创作段落标记来源失效并可重建/删除。
  - `AC-FR-DIA-003-03`：Given 生成失败或碎片为空，When 提交，Then 保留原始输入并提示可重试，不虚构内容。
- **测试**：`TC-APAT-DIA-001`、`TC-PRIV-DIA-001`。
- **依赖**：步 2（次日梦境开场白）依赖 `CAP-030` 主动闲聊与 `CAP-018` 表现层，见 FR-CONV-006 依赖说明，不单独结算。

### FR-CONV-006 昨日会话复盘（二周目回溯，拆步 1）

- **Parent CAP**：`CAP-014`
- **触发**：用户对昨日会话发起复盘。
- **必须**：展示昨日会话的关键节点（问题、作答、复习项）并以第三视角回顾；Agent 给出复盘建议（"如果重来可考虑…"）且标注为推断；复盘基于已持久化 Turn/MessageVersion 数据，不得引用已删除或撤权内容；完整分支回溯依赖会话分支数据结构（CAP-014 主能力），未落地前仅提供线性回顾。
- **异常**：昨日无会话、来源删除、生成失败。
- **数据**：复盘为只读视图与生成文本，不改变会话真源。
- **验收**：
  - `AC-FR-CONV-006-01`：Given 昨日存在会话，When 用户发起复盘，Then 展示关键节点与来源时间线。
  - `AC-FR-CONV-006-02`：Given 会话含删除/撤权内容，When 复盘，Then 不展示其正文。
  - `AC-FR-CONV-006-03`：Given Agent 给出建议，When 展示，Then 明确标记为推断意见而非事实记录。
- **测试**：`TC-INTEG-CONV-002`、`TC-PRIV-CONV-001`。
- **依赖**：分支级"倒带"依赖 `CAP-014` 会话地图完整能力；"次日开场白"依赖 `CAP-030`，本步只做被动复盘。

### FR-MEM-003 外部文档记忆消化（记忆外挂，拆步 1）

- **Parent CAP**：`CAP-005`、`CAP-020`
- **触发**：用户经插件注入长文本（PDF/网页链接）。
- **必须**：内容解析后作为**候选**记忆写入（复用 `memory_embeddings` 与来源链），默认 `verificationStatus=unverified`；用户可查看、修正、删除注入内容；后续对话可在授权窗口内检索到注入内容；引用必须回指原文档来源；不得因注入自动晋升高层记忆。
- **异常**：解析失败、版权/许可证未确认、注入超限、来源失效。
- **数据**：`memory_records` 候选 + 来源引用；遵守 `BR-LRN-001` 版权规则。
- **验收**：
  - `AC-FR-MEM-003-01`：Given 用户注入长文本，When 解析完成，Then 以未验证候选写入并可在对话中检索，引用可回指原文。
  - `AC-FR-MEM-003-02`：Given 注入内容许可证未确认，When 处理，Then 标记受限，不作为可信来源传播。
  - `AC-FR-MEM-003-03`：Given 用户删除注入内容，When 清理，Then 关联引用失效并重建检索索引（24 小时内）。
- **测试**：`TC-INTEG-MEM-003`、`TC-PRIV-DEL-001`。
- **依赖**：步 2（三日随机引用）依赖 `CAP-030`，不单独结算。

### FR-PLG-001 心流状态机（人格切换）

- **Parent CAP**：`CAP-020`、`CAP-019`
- **触发**：用户接入自有 API Key 或启用本地模型 Provider 后切换"人格模式"。
- **必须**：基于既有流动性 `Persona` 字段化与 Provider Port：接入 SDK Key/本地模型走既有 Provider 适配（`ADR-005`），人格切换改变对话风格与界面主题（如编程护目镜/终端配色、写作书生风）；主题切换为纯表现层，不改变权限、数据或安全边界；允许多人格间切换并在默认兜底人格下工作。
- **异常**：Key 无效/限额、Provider 不可用、切换失败、外部依赖故障不阻断核心对话。
- **数据**：Key 仅存本机与加密存储；人格配置复用 `persona` schema。
- **验收**：
  - `AC-FR-PLG-001-01`：Given 用户接入有效 Key，When 开启人格模式，Then 对话行为与界面主题切换成功，默认兜底仍可用。
  - `AC-FR-PLG-001-02`：Given Key 失效或服务不可用，When 请求，Then 降级到默认 Provider 与默认人格并明确提示，不影响核心链路。
  - `AC-FR-PLG-001-03`：Given 人格模式仅切换表现与风格，When 执行，Then 权限、数据范围与安全边界不变化。
- **测试**：`TC-API-PLG-001`、`TC-SEC-PLG-001`。

### FR-UX-005 无 UI 陪伴模式（Zen Mode，拆步 1）

- **Parent CAP**：`CAP-018`
- **触发**：用户在首屏空白处双击进入无 UI 模式。
- **必须**：双态切换隐藏全部工作台卡片，仅保留对话输入与桌宠/陪伴主体（Web 对应工作台状态卡）；再次双击或 Esc 退出恢复原态；模式切换为纯前端状态，不改变会话或数据；主动闲聊对话依赖 `CAP-030`，本步不承诺待命自动发问。
- **异常**：键盘可达性、恢复失败、无障碍用户被隐藏输入阻断。
- **数据**：无新增数据。
- **验收**：
  - `AC-FR-UX-005-01`：Given 用户双击空白，When 进入，Then 隐藏全部卡片仅保留对话与陪伴主体，键盘可操作。
  - `AC-FR-UX-005-02`：Given 用户双击或 Esc，When 退出，Then 恢复原工作台且会话状态不丢失。
  - `AC-FR-UX-005-03`：Given 无障碍用户，When 使用，Then 无 UI 模式不阻断输入与恢复路径。
- **测试**：`TC-E2E-UX-001`、`TC-A11Y-CORE-001`。
- **依赖**：等待式"主动发问"依赖 `CAP-030` 主动提醒深化，本步只做模式壳。
