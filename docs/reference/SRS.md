...
- **异常**：空字段、非法时长、重复提交、本地数据目录不可写、写入超时。
...
- **Parent CAP**：`CAP-006`
- **规则**：MVP 答错或连续答对数为 0 时次日复习；答对后按更新后的连续答对数 1/2/3/4+ 分别安排 2/4/8/15 天后复习。v1 使用固定 24 小时时长；v2 按完成时的 IANA 时区增加本地日历天、保存 `timezoneSnapshot` 并保留墙上时间。完成复习必须在同一事务中完成当前项、更新知识点状态并创建下一条活动项；同一知识点只保留一条活动复习项。
...
- **一致性**：`Diary` 对 `(localDate,autoGenerated=true)` 唯一；`DiaryMaterialBuffer` 只能关联 `cycleId`。`DiaryRunAttempt` 用 schedule revision、lease 和 fencing token 拒绝旧 Worker；只有 `Published` 或有审计原因的 `Skipped` 才能与 `lastCutoffAt/cursorVersion` 在同一事务提交，`Failed/Cancelled` 不推进 cursor。计划修订遇到已有自动日记的同日标签时顺延到下一未占用标签。
...
### SEC-LOC-001 本地单用户数据与 API 边界

- **Parent CAP**：`CAP-001`～`CAP-035`
- **必须**：个人业务数据归属于当前操作系统用户的数据目录；API 默认只监听 loopback。非 loopback 监听必须显式开启并强制 token 认证，`open` 认证与非 loopback 组合必须在启动期 fail closed。插件和外部连接器保留独立 `actorId`、Grant、来源与审计，不得扩大文件、工具或网络权限。
- **验收**：
  - `AC-SEC-LOC-001-01`：Given 默认配置，When API 启动，Then 仅监听 `127.0.0.1`/`::1`。
  - `AC-SEC-LOC-001-02`：Given `open` 认证与非 loopback 地址，When API 启动，Then 启动失败且不开放端口。
  - `AC-SEC-LOC-001-03`：Given 第二个操作系统用户读取数据目录，When 权限检查，Then 默认 ACL/权限拒绝访问。
  - `AC-SEC-LOC-001-04`：Given 插件或外部内容请求扩大权限，When 执行，Then ToolPolicy/Grant 拒绝并写入最小审计。
- **测试**：`TC-SEC-LOCAL-API-001`、`TC-SEC-LOCAL-FS-001`、`TC-SEC-PLUG-001`。
...
| `NFR-SEC-001` | 全部生命周期能力遵循 OWASP ASVS L2、本机 loopback 默认、非 loopback 强制认证、操作系统用户文件隔离、最小权限、供应链扫描、流式撤权和审计；任何未授权远程访问或高危供应链/许可证问题阻断发布 |
...
> [!NOTE]
> 本节需求规格已完整拆分至专属子规格文档：[主动智能与外部信号需求规格（AVX-SRS-002）](srs-proactive-intelligence.md)。
> 包括 CAP-033（全域感知与个人画像/主动智能模式）的授权、观察、画像推断、后台生命周期、主动动作、撤权删除、数据实体与安全质量要求。
> 详见 [srs-proactive-intelligence.md](srs-proactive-intelligence.md#1-cap-033-全域感知与个人画像主动智能模式)。
...
- 详细规格请跳转查阅：[srs-proactive-intelligence.md](srs-proactive-intelligence.md#1-cap-033-全域感知与个人画像主动智能模式)。

## 9. CAP-033～035 主动智能派生与外部信号

> [!NOTE]
> 本节需求规格（FR-PRO-013 主动智能派生、FR-HA-001/002 Home Assistant 连接与执行、FR-HEALTH-001 小米运动健康同步、PRIV-EXT-001 外部连接凭据最小化）已完整拆分至：[主动智能与外部信号需求规格（AVX-SRS-002） §2](srs-proactive-intelligence.md#2-cap-033035-主动智能派生与外部信号)。
> 详见 [srs-proactive-intelligence.md](srs-proactive-intelligence.md#2-cap-033035-主动智能派生与外部信号)。

<!-- 保持兼容锚点以供既有文档与追踪矩阵索引 -->
<a id="fr-pro-013-十二项主动智能派生"></a>
<a id="fr-ha-001-home-assistant-连接与感知"></a>
<a id="fr-ha-002-home-assistant-受控执行"></a>
<a id="fr-health-001-小米运动健康按日同步"></a>
<a id="priv-ext-001-外部连接凭据与数据最小化"></a>

- 详细规格请跳转查阅：[srs-proactive-intelligence.md](srs-proactive-intelligence.md#2-cap-033035-主动智能派生与外部信号)。
