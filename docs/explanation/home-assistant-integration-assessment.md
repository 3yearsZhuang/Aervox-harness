...
建议以设备/家庭环境专属的新 CAP 承载（编号须在后续 CR 阶段单独裁定），不占用已登记的 `CAP-033`，也不静默扩写 `CAP-018/027`；若判定为纯工具集，也可暂不建 CAP、随 Loop 工具注册表演进，由 CR 裁决。
...
5. HA 版本漂移与实体兼容策略（HA 以月度节奏发布新版本，跨版本 API 稳定但集成行为可能变化）。
