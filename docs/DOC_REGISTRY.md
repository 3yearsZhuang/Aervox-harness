...
| `AVX-SRS-001` | [SRS](reference/SRS.md) | 2026-09-10 | G1 需求基线前 | 版本内 FR/BR/AC 变化未同步或未过 DoR |
...
| `ADR-003` | [SQLite 业务真源与 FTS5/Vector Port 仓储抽象](reference/adr/ADR-003-postgres-retrieval.md) | 2026-09-10 | G2 评审 + 决策变更 | 决策被 Superseded/Rejected 未登记 |
...
| `ADR-008` | [Cloud-First 架构与本地存储 Port 预留](reference/adr/ADR-008-cloud-first-local-port.md) | 2026-09-10 | G2 评审 + 决策变更 | 决策被 Superseded/Rejected 未登记 |
...
| `AVX-DB-001` | [数据库设计与双引擎契约](reference/DATABASE.md) | 2026-09-10 | Schema/仓储接口/迁移计划变更 | 仓储接口签名或租户隔离模式/PG 切换计划与实现不一致 |
| `AVX-DB-002` | [数据库数据模型覆盖矩阵](reference/database-coverage-matrix.md) | 2026-09-10 | PRD §8 实体增减或落表状态变更 | 实体落表状态、仓储 Port 或 DDL 初始化与实现不符 |
...
| `AVX-PLUG-001` | [插件 Config、Page 与 UI 扩展规范](reference/plugin-config-and-pages.md) | 2026-09-10 | CR-006 / 插件机制变更 | Manifest、Config Schema、Page Bridge、Turn 管道或 UI 扩展与实现不一致 |
...
| `AVX-TUT-002` | [教程：迁移已集成能力并接入 DSH/pi](tutorials/migrate-integrated-capabilities.md) | 2026-08-31 | 能力目录、DSH/pi 上游或迁移步骤变更 | 当前实现路径、固定 SHA、权限/隔离边界或验证命令与仓库不一致 |
...
| `AVX-DOC-CONF-001` | [文档生命周期登记表](DOC_REGISTRY.md) | 2026-09-10 | 每季度 + 每次文档集变更 | 登记条目与实际文档集不一致 |
